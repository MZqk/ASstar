"""
Siril ImageWindow - A pseudo-MDI GUI script for storing and swapping images with Siril

# (c) Adrian Knagg-Baugh 2025-2026
...
# 1.0.4  Fix some bugs reported by Deep Space Astro. Thanks Rich!
# 1.0.5  Improve numba importing (add numpy version constraint)
"""
...
s.ensure_installed("numpy", version_constraints="<2.5.0")
...
class SirilImageHolder(QMainWindow):
    """Main application window for the Siril ImageWindow script"""
        ...
        if self.siril:
            with self.siril_mutex:
                ...
                if is_sequence or is_image:
                    self.stf = self.siril.get_siril_stf()
                    self.display_combo.setCurrentIndex(self.stf) # Match Siril STF
    ...
    def init_ui(self):
        ...
        self.setWindowTitle("Siril ImageWindow - No image")
        ...
        self.linked_checkbox.setEnabled(self.stf == s.STFType.AUTOSTRETCH_DISPLAY)
    ...
    def open_from_file(self):
        ...
        try:
            ...
            filename, _ = QFileDialog.getOpenFileName(
                ...
                "Open Image File",
                "",
                "Image Files (*.fit *.fits *.fts *.tif *.tiff *.xisf);;All Files (*)"
            )
            ...
            self.setWindowTitle(f"Siril ImageWindow - {self.stored_filename}")
    ...
    def update_display_combo_from_sync(self, stf_value, linked):
        ...
        if self.sync_mode == "Follow Siril":
            ...
            self.linked_checkbox.setEnabled(stf_value == s.STFType.AUTOSTRETCH_DISPLAY)
        ...
        try:
            with self.siril_mutex:
                ...
                if self.siril.is_image_loaded():
                    ...
                    self.siril.cmd("new", "1", "1", "1", self.stored_filename \
                            if isinstance(self.stored_filename, str) else "New image")
    ...
    def apply_display_transform(self, image_data, mode):
        ...
        if (mode == "Linear" and
            ...
            self.display_icc_profile is not None and
...
def main():
    ...
    app.setApplicationName("Siril ImageWindow")
