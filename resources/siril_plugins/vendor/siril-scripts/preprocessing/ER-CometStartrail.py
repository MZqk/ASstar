...
# 1.0.5 Updated calls to `set_seq_frame_pixeldata` to include the new `prefix` parameter, as required by the modified Python API introduced in version 1.4.0.
# 1.0.6 Applies effect only to stars by using starless subtraction, eliminating the previous user‑defined step
...
class CometStarTrailsInterface:
    ...
    def apply_effect_worker(self):
        while True:
            ...
            with fits.open(self.siril.get_seq_frame_filename(index-1), mode='update') as hdul:
                ...
                frame_with_effect = 1 - ((1 - ((data_f - starless_data) * value)) * (1 - starless_data))
                frame_with_effect = np.maximum(frame_with_effect, starless_data)
                frame_with_effect = np.clip(frame_with_effect, 0.0, 1.0)
                starless_with_effect = 1 - ((1 - ((starless_data - starless_data) * value)) * (1 - starless_data))
                starless_with_effect = np.maximum(starless_with_effect, starless_data)
                starless_with_effect = np.clip(starless_with_effect, 0.0, 1.0)
                result = (frame_with_effect - starless_with_effect) + starless_data
    ...
    def show_info(self):
        messagebox.showinfo(
            "Comet Startrails - Usage",
            (
                "🔹 Adjust the initial and final effects for the star trails:\n"
                "   • Soften: Gradually fade in/out the trails\n"
                "   • Enhance: Gradually intensify the trails\n\n"
                "🔹 Use the slider to set how many frames the effect will be applied to.\n\n"
                "🔹 You can exclude frames from the sequence using *Sequence → Frame List* in Siril.\n\n"
                "🔹 Click 'Apply' to start processing the startrail with your chosen effects.\n"
            )
        )
