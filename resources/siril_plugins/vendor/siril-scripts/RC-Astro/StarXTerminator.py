...
read live from the schema (e.g. --stars/--no-stars, --unscreen, --overlap):
...
PREFIX_SXT = "sxt_"
# When the "Generate Stars Image" option is on, rc-astro writes the stars-only
# image beside the -o output as "<output_base>-stars.<ext>". The script saves it
# into Siril's working directory, prefixing the loaded image / sequence name with
# STARS_PREFIX (so a sequence yields a parallel 'stars_' sequence).
STARS_RC_SUFFIX = "-stars"   # how rc-astro names its sidecar file
STARS_PREFIX = "stars_"      # prefix for the stars image / sequence in Siril's wd
...
DEFAULT_SCHEMA = {
    ...
    "parameters": [
        ...
        {"label": "Generate Stars Image", "name": "stars", "flag": "--stars",
         "description": "Also write a stars-only image (original minus starless) "
                        "beside the starless output",
         "type": "bool", "default": False},
        {"label": "Unscreen Stars", "name": "unscreen", "flag": "--unscreen",
         "description": "Unscreen stars (requires --stars)",
         "type": "bool", "default": False},
    ],
    "groups": [
        {"name": "optionsGroup", "label": "Options", "params": ["stars", "unscreen"]},
...
def run_sxt(exe, input_fits, output_fits, param_args, mlversion, depth,
    ...
    file — e.g. sxt --stars — emits a saving/complete pair per file, told apart by
    each status event's `output`.)
...
def process_array(arr, opts, schema, progress_cb=None, log_cb=None,
    ...
    When the "Generate Stars Image" option produced a stars-only sidecar and
    ...
    try:
        ...
        result = run_sxt(exe, in_fits, out_fits, param_args, opts.mlversion,
        ...
        if stars_cb is not None:
            base, ext = os.path.splitext(out_fits)
            stars_fits = base + STARS_RC_SUFFIX + ext
            if os.path.isfile(stars_fits):
                stars_cb(read_working_fits(stars_fits, arr.dtype))
        return starless
    finally:
...
def process_sequence(siril, opts, schema, progress_cb=None, log_cb=None,
    ...
    for i in range(n):
        ...
        # Capture this frame's stars image (if "Generate Stars Image" is on) and
        # write it as the matching frame of a parallel 'stars_' sequence.
        captured = {}
