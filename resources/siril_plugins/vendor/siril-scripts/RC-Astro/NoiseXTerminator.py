...
"""
NoiseXTerminator denoising for Siril.

A self-contained wrapper around RC-Astro's command-line tool (the ``nxt``
subcommand of the ``rc-astro`` executable). It denoises the currently loaded
image or a loaded FITS sequence and loads the result back into Siril.

The set of processing options, together with their ranges and defaults, is read
live from ``rc-astro nxt --json`` so the script stays in step with the tool. A
...
NoiseXTerminator is a separate, commercial program by Russell Croman
...
read live from the schema (e.g. --dn, --it, --overlap):
    pyscript NoiseXTerminator.py --dn 0.8
    pyscript NoiseXTerminator.py --sequence --dn 0.6 --it 3
    pyscript NoiseXTerminator.py --dn 0.5 --overlap 0.3
...
# Version history
# 1.0.0 - Initial release: NoiseXTerminator (rc-astro nxt) support, PyQt6 GUI +
#         CLI, single image and FITS sequence processing, schema driven from
#         'nxt --json' (unified schema v3), native bit-depth FITS exchange,
...
#         (its AI2 lacks the iteration and separation controls).
...
VERSION = "1.0.5"
PRODUCT_LABEL = "NoiseXTerminator"
CONFIG_SUBDIR = "NoiseXTerminator"
CONFIG_FILENAME = "settings.json"
NXT_WEBSITE = "https://www.rc-astro.com"
# rc-astro is a multi-tool; NoiseXTerminator is the 'nxt' subcommand.
SUBCOMMAND = "nxt"
NXT_EXE_NAMES = ("rc-astro", "rc-astro.exe", "RC-Astro", "RC-Astro.exe")
PREFIX_NXT = "nxt_"
...
# Built-in fallback schema (a copy of 'rc-astro nxt --json', schemaVersion 4),
...
DEFAULT_SCHEMA = {
    ...
    "parameters": [
        ...
        {"label": "Denoise", "name": "dn", "flag": "--dn",
         "description": "Denoise strength",
         "type": "float", "precision": 2, "default": 0.0, "min": 0.0, "max": 1.0,
         "visibleIf": "!csep && !fsep"},
        {"label": "Color Separation", "name": "csep", "guiOnly": True,
         "description": "Process luminance and color independently",
         "type": "bool", "default": False},
        {"label": "Denoise Intensity", "name": "di", "flag": "--di",
         "description": "Intensity denoise strength",
         "type": "float", "precision": 2, "default": 0.0, "min": 0.0, "max": 1.0,
         "visibleIf": "csep && !fsep"},
        {"label": "Denoise Color", "name": "dc", "flag": "--dc",
         "description": "Chrominance denoise strength",
         "type": "float", "precision": 2, "default": 0.0, "min": 0.0, "max": 1.0,
         "visibleIf": "csep && !fsep"},
        {"label": "Frequency Separation", "name": "fsep", "guiOnly": True,
         "description": "Apply denoising in a frequency-separated manner",
         "type": "bool", "default": False},
        {"label": "Denoise High Frequency", "name": "dhf", "flag": "--dhf",
         "description": "High-frequency (small scale) denoise strength",
         "type": "float", "precision": 2, "default": 0.0, "min": 0.0, "max": 1.0,
         "visibleIf": "!csep && fsep"},
        {"label": "Denoise Low Frequency", "name": "dlf", "flag": "--dlf",
         "description": "Low-frequency (large scale) denoise strength",
         "type": "float", "precision": 2, "default": 0.0, "min": 0.0, "max": 1.0,
         "visibleIf": "!csep && fsep"},
        {"label": "Denoise Intensity High Frequency", "name": "dihf", "flag": "--dihf",
         "description": "High-frequency (small scale) intensity denoise strength",
         "type": "float", "precision": 2, "default": 0.0, "min": 0.0, "max": 1.0,
         "visibleIf": "csep && fsep"},
        {"label": "Denoise Intensity Low Frequency", "name": "dilf", "flag": "--dilf",
         "description": "Low-frequency (large scale) intensity denoise strength",
         "type": "float", "precision": 2, "default": 0.0, "min": 0.0, "max": 1.0,
         "visibleIf": "csep && fsep"},
        {"label": "Denoise Color High Frequency", "name": "dchf", "flag": "--dchf",
         "description": "High-frequency (small scale) color denoise strength",
         "type": "float", "precision": 2, "default": 0.0, "min": 0.0, "max": 1.0,
         "visibleIf": "csep && fsep"},
        {"label": "Denoise Color Low Frequency", "name": "dclf", "flag": "--dclf",
         "description": "Low-frequency (large scale) color denoise strength",
         "type": "float", "precision": 2, "default": 0.0, "min": 0.0, "max": 1.0,
         "visibleIf": "csep && fsep"},
        {"label": "Frequency Scale", "name": "fs", "flag": "--fs",
         "description": "Pixel scale of the low/high frequency transition band",
         "type": "float", "precision": 1, "default": 5.0, "min": 1.0, "max": 100.0,
         "logarithmic": True, "visibleIf": "fsep", "unit": "pixels"},
        {"label": "Iterations", "name": "it", "flag": "--it",
         "description": "Number of denoising iterations to perform",
         "type": "int", "default": 2, "min": 1, "max": 5},
    ],
    "groups": [
        {"name": "optionsGroup", "label": "Options", "params": ["csep", "fsep"]},
        {"name": "denoiseGroup", "label": "Denoise",
         "params": ["dn", "di", "dc", "dhf", "dlf", "dihf", "dilf",
                    "dchf", "dclf", "fs"]},
        {"name": "engineGroup", "label": "Engine", "params": ["it", "overlap"]},
    ],
}


# ---------------------------------------------------------------------------
# Schema helpers (generic; nothing here is nxt-specific)
# ---------------------------------------------------------------------------
...
def normalize_schema(data):
    """Validate and lightly normalise a 'nxt --json' document. Parameters,
...
def about_html():
    ...
    return (
        "<h3>NoiseXTerminator for Siril</h3>"
        f"<p>Version {VERSION}</p>"
        "<p>NoiseXTerminator is a third-party neural-network tool by Russell "
        "Croman (RC-Astro) that reduces noise in astronomical images while "
        "preserving fine detail. This script drives the RC-Astro command-line "
        "tool from Siril and loads the result back.</p>"
        "<p>The available options and their ranges are read directly from the "
        "tool, so they stay current as NoiseXTerminator is updated.</p>"
        "<p>NoiseXTerminator is a separate, commercial product and must be "
        ...
        f'<p>Website: <a href="{NXT_WEBSITE}">{NXT_WEBSITE}</a></p>'
    )
...
# colour, dtype uint16 or float32. NoiseXTerminator handles full 32-bit FITS, so
...
# out cancels that (NoiseXTerminator's processing is orientation independent).
...
def find_nxt_on_path():
    for name in NXT_EXE_NAMES:
...
def fetch_schema(configured, mlversion=0):
    """Probe 'rc-astro nxt --ml-version N --json' (no input file = parameter
    ...
    try:
        ...
        if data.get("key") == SUBCOMMAND or "parameters" in data:
            ...
            name = data.get("name", "NoiseXTerminator")
            msg = f"{name} detected (model v{schema['mlVersion']})."
            return ("ok", msg, schema)
    except (ValueError, TypeError):
        pass
    return ("unknown",
            "Could not read the NoiseXTerminator parameter schema; using built-in defaults.",
            DEFAULT_SCHEMA)
...
def run_nxt(exe, input_fits, output_fits, param_args, mlversion, depth,
            progress_cb=None, log_cb=None, cancel_cb=None, device="auto",
            use_device=True):
    """Run 'rc-astro nxt' on input_fits producing output_fits. Raises NxtError on
    ...
    with _nxt_lock:
        try:
            ...
            raise NxtError(f"Could not launch rc-astro: {exc}") from exc
    ...
    if not os.path.isfile(output_fits):
        raise NxtError("NoiseXTerminator did not produce an output file.")
    return True
...
def process_array(arr, opts, schema, progress_cb=None, log_cb=None,
                  cancel_cb=None):
    """Run NoiseXTerminator on a single Siril pixel array. Returns the processed
...
def process_single_image(siril, opts, schema, progress_cb=None, log_cb=None,
                         cancel_cb=None):
    """Run NoiseXTerminator on the loaded image and load the result back."""
    ...
    if result is None:
        return False
    siril.undo_save_state("NoiseXTerminator")
...
def process_sequence(siril, opts, schema, progress_cb=None, log_cb=None,
                     cancel_cb=None):
    """Run NoiseXTerminator on every selected frame of the loaded FITS sequence,
    building a new 'nxt_' sequence."""
    seq = siril.get_seq()
    ...
    for i in range(n):
        ...
        if result is None:
            return False
        siril.set_seq_frame_pixeldata(i, result, prefix=PREFIX_NXT)
        if progress_cb:
            progress_cb((i + 1) / n)
    ...
    # the prefix alone. Passing only PREFIX_NXT matches no files and no .seq is
    # created (issue #116); build the full root from the source sequence name.
    siril.create_new_seq(PREFIX_NXT + Path(seq.seqname).name)
    if log_cb:
...
def load_options(siril):
    try:
        ...
        return NxtOptions.from_dict(data)
...
def save_options(siril, opts):
    try:
        ...
        siril.log(f"NoiseXTerminator: could not save settings: {exc}", LogColor.SALMON)
    ...
    def run(self):
        try:
            ...
            if self._cancel:
                ...
                self.finished_ok.emit(True, "NoiseXTerminator finished.")
            else:
                self.finished_ok.emit(False, "NoiseXTerminator did not complete.")
        except Exception as exc:
...
class NxtGUI(QMainWindow):
    def __init__(self, siril):
        ...
        self.setWindowTitle(f"NoiseXTerminator v{VERSION}")
        self._build_ui()
    ...
    def _build_ui(self):
        ...
        about_btn.setToolTip("About NoiseXTerminator and this script.")
        ...
        self.run_btn = QPushButton("Run NoiseXTerminator")
    ...
    def _show_about(self):
        dlg = QDialog(self)
        dlg.setWindowTitle("About NoiseXTerminator")
        lay = QVBoxLayout(dlg)
    ...
    def _on_run(self):
        ...
        self.worker = NxtWorker(self.siril, opts, self.schema, sequence_mode)
    ...
    def _on_progress(self, frac):
        ...
        try:
            self.siril.update_progress("NoiseXTerminator", max(0.0, min(1.0, frac)))
        except Exception:
            pass
    ...
    def _on_finished(self, ok, message):
        self.run_btn.setText("Run NoiseXTerminator")
        self.run_btn.setEnabled(True)
        self.progress_bar.setRange(0, 100)
        ...
        if ok:
            ...
            QMessageBox.warning(self, "NoiseXTerminator", message)
...
def build_schema_parser(schema):
    ...
    p = argparse.ArgumentParser(
        prog="NoiseXTerminator.py",
        description="Denoise the loaded image or sequence using NoiseXTerminator.",
...
def run_cli(siril, argv):
    ...
    siril.log(f"NoiseXTerminator: {message}",
    ...
    def progress_cb(frac):
        try:
            siril.update_progress("NoiseXTerminator", max(0.0, min(1.0, frac)))
        except Exception:
            pass

    def log_cb(text):
        siril.log(f"NoiseXTerminator: {text}", LogColor.DEFAULT)

    try:
        if args.sequence:
            if not siril.is_sequence_loaded():
                siril.log("NoiseXTerminator: no sequence is loaded.", LogColor.RED)
                return 1
            ok = process_sequence(siril, opts, schema, progress_cb=progress_cb,
                                  log_cb=log_cb)
        else:
            if not siril.is_image_loaded():
                siril.log("NoiseXTerminator: no image is loaded.", LogColor.RED)
                return 1
            ok = process_single_image(siril, opts, schema, progress_cb=progress_cb,
                                      log_cb=log_cb)
    except NxtError as exc:
        siril.log(f"NoiseXTerminator: {exc}", LogColor.RED)
        return 1
    finally:
        try:
...
def main():
    ...
    try:
        ...
        print("NoiseXTerminator: could not connect to Siril.", file=sys.stderr)
        sys.exit(1)
