...
syqon_prism_inference.py is required. Model weights (.pt files) must
be present in the syqon_prism directory.

Usage:
    GUI mode (default):
        Add this script to the Scripts menu and run it. Ensure the model
        file is present in the syqon_prism directory.
...
v2.2.0  SyQon Studio Stretch integration and UI overhaul:
        - Integrated SyQon Studio stretch algorithms (Statistical & IHS Aggressive)
        - Optimized statistical stretch calculation via 100k pixel sampling
        - Responsive UI debounce timer (120ms) for preview updates
        - Weight status indicators (✓ / ✗) and built-in local model file importer
        - Disabled Prism Mini selection in GUI to default to Prism Deep
        - Removed automatic model downloads
        - Complete UI visual overhaul with Teal/Emerald theme and Rangefinder-style selection
v2.3.0  Prism Mini restoration, Model Management tab & Multi-platform Hardware Acceleration:
        - Restored Prism Mini as an active, selectable model option in GUI and config
        - Added dedicated "Models" tab with visual status indicators (Installed / Missing)
        - Implemented individual "Load..." & "Remove" model installation and management controls
        - Added direct web links for "Get Free Models" (syqon.eu/free-prism-mini) and "Get Deep Prism" (syqon.eu/prism)
        - Multi-platform GPU acceleration for AMD (ROCm, Metal MPS, DirectML) for both Mini and Deep models
        - Enhanced Intel Mac support (Metal MPS GPU detection with vecLib/OpenMP CPU fallback)
        - Integrated AI model memory cache clearing utility ("Clear AI Cache")
"""
...
from PySide6.QtWidgets import (
    ...
    QFrame, QGroupBox, QDoubleSpinBox, QTabWidget,
)
...
PRISM_DEEP_URL         = "https://syqon.eu/prism"
FREE_PRISM_MODELS_URL  = "https://syqon.eu/free-prism-mini"
...
DEFAULT_CONFIG = {
    ...
    "stretch_target": 0.25,
    "ihs_target": 0.25,  # fallback compatibilità
}
...
def ensure_mini_model(engine_dir: Path, siril,
    ...
    """
    Ensure prism_mini.pt is present in the syqon_prism directory.
    ...
    if not model_file.exists():
        print(f"Prism Mini model file '{file_name}' not found at {engine_dir}.")
        return model_file
...
def get_device(use_gpu: Optional[bool] = True) -> torch.device:
    ...
    # 1. CUDA / AMD ROCm (NVIDIA / AMD ROCm on Linux or Windows)
    if torch.cuda.is_available():
        try:
            dev_name = torch.cuda.get_device_name(0) if torch.cuda.device_count() > 0 else "CUDA/ROCm"
            print(f"Using CUDA / AMD ROCm GPU ({dev_name})")
        except Exception:
            print("Using CUDA / AMD ROCm GPU")
        return torch.device("cuda")

    # 2. MPS (Apple Silicon & Intel Mac AMD GPU via Metal)
    if hasattr(torch.backends, "mps") and torch.backends.mps.is_available():
        try:
            test_t = torch.zeros(1, device="mps")
            del test_t
            mac_arch = "Apple Silicon" if platform.machine() == "arm64" else "Intel Mac GPU"
            print(f"Using Metal MPS GPU ({mac_arch})")
            return torch.device("mps")
        except Exception as exc:
            print(f"Metal MPS not available on this Mac GPU ({exc}) — falling back to CPU")

    # 3. DirectML (AMD / Intel / NVIDIA GPUs on Windows via DirectX)
    try:
        ...
        if torch_directml.is_available():
            ...
            try:
                ...
                print(f"Using AMD / DirectML GPU ({gpu_name})")
            except Exception:
    ...
    # 5. CPU (Intel Mac / x86_64 or ARM64)
    if platform.system() == "Darwin" and platform.machine() != "arm64":
        print("Using Intel Mac CPU (Accelerated via vecLib/OpenMP)")
    else:
...
def load_model(use_gpu: Optional[bool] = True,
    ...
    d_type = device.type if isinstance(device.type, str) else str(device.type)
    amp_enabled   = d_type in ("cuda", "mps", "xpu")
    print(f"Residual mode: {residual_mode}, Device: {device}, AMP: {amp_enabled}")
...
# ---------------------------------------------------------------------------
# Statistical Stretch (Auto) matching SyQon Studio
# ---------------------------------------------------------------------------

def pi_mtf(x: np.ndarray, Med: float | np.ndarray, T: float) -> np.ndarray:
    num = (Med - 1.0) * T * x
    denom = Med * (T + x - 1.0) - T * x
    denom = np.where(np.abs(denom) < 1e-12, np.where(denom < 0, -1e-12, 1e-12), denom)
    return num / denom


def pi_mtf_inv(y: np.ndarray, Med: float | np.ndarray, T: float) -> np.ndarray:
    num = y * Med * (1.0 - T)
...
def compute_statistical_params(data: np.ndarray, target: float) -> dict:
    chw, _ = _to_chw(data.astype(np.float32))
    bp_list = []
    scale_list = []
    med2_list = []
    ...
    for i in range(chw.shape[0]):
        ch = chw[i]
        # Campionamento per velocizzare drasticamente su immagini grandi
        ...
        if abs(scale) < 1e-12:
            scale = 1e-12
        x_bp = np.clip((sample - bp) / scale, 0.0, 1.0)
        med2 = max(np.median(x_bp), 1e-6)
        
        bp_list.append(bp)
        scale_list.append(scale)
        med2_list.append(med2)
...
def apply_statistical_stretch(data: np.ndarray, params: dict, target: float) -> np.ndarray:
    chw, was_hwc = _to_chw(data.astype(np.float32))
    mono = chw.shape[0] == 1
    result = np.empty_like(chw)
    for i in range(chw.shape[0]):
        idx = min(i, len(params['bp']) - 1)
        bp = params['bp'][idx]
        scale = params['scale'][idx]
        med2 = params['med2'][idx]
        rescaled = np.clip((chw[i] - bp) / scale, 0.0, 1.0)
        result[i] = np.clip(pi_mtf(rescaled, med2, target), 0.0, 1.0)
    return _to_hwc(result, was_hwc, mono)


def apply_statistical_stretch_inverse(data: np.ndarray, params: dict, target: float) -> np.ndarray:
    chw, was_hwc = _to_chw(data.astype(np.float32))
    mono = chw.shape[0] == 1
    result = np.empty_like(chw)
    for i in range(chw.shape[0]):
        ...
        scale = params['scale'][idx]
        med2 = params['med2'][idx]
        unstretched = pi_mtf_inv(chw[i], med2, target)
        unstretched = np.clip(unstretched, 0.0, 1.0)
        result[i] = unstretched * scale + bp
    return _to_hwc(result, was_hwc, mono)


# ---------------------------------------------------------------------------
# Unified Stretch Wrapper
# ---------------------------------------------------------------------------

def compute_stretch_params(data: np.ndarray, method: str, target: float) -> dict:
    if method == "ihs":
        return compute_ihs_params(data, target=target)
    else:
        return compute_statistical_params(data, target=target)


def apply_stretch(data: np.ndarray, method: str, params: dict, target: float) -> np.ndarray:
    if method == "ihs":
        return apply_ihs(data, params)
    else:
        return apply_statistical_stretch(data, params, target)


def apply_stretch_inverse(data: np.ndarray, method: str, params: dict, target: float) -> np.ndarray:
    if method == "ihs":
        return apply_ihs_inverse(data, params)
    else:
        return apply_statistical_stretch_inverse(data, params, target)
...
def tile_inference_torch(
    ...
    d_type = device.type if isinstance(device.type, str) else str(device.type)
    autocast_dev = d_type if d_type in ("cuda", "mps", "xpu", "cpu") else "cpu"
    autocast_on = amp_enabled and (d_type in ("cuda", "mps", "xpu"))
    ...
    if tile_size <= 0 or tile_size >= min(h, w):
        ...
        try:
            with torch.no_grad(), amp.autocast(device_type=autocast_dev, enabled=autocast_on):
                result = model(x)
        except Exception:
            with torch.no_grad():
                result = model(x)
    ...
    with torch.no_grad():
        for y in reversed(ys):
            ...
            for x0 in xs:
                ...
                try:
                    with amp.autocast(device_type=autocast_dev, enabled=autocast_on):
                        pred = model(tile)
                except Exception:
                    pred = model(tile)
...
def process_image(
    ...
    ihs_target: Optional[float] = None,
) -> np.ndarray:
    """
    Denoise a single image using tiled NAFNet + statistical/IHS autostretch.
    """
    def _emit(v):
        if progress_callback:
            progress_callback(v)
    ...
    if ihs_target is not None:
        stretch_target = ihs_target
        stretch_method = "ihs"

    stretch_params = compute_stretch_params(img, stretch_method, stretch_target)
    ...
    img          = apply_stretch(img, stretch_method, stretch_params, stretch_target)
    _emit(5)
    ...
    result = apply_stretch_inverse(result, stretch_method, stretch_params, stretch_target)
...
class PrismWorker(QObject):
    ...
    def __init__(self, raw: np.ndarray, tile: int, overlap: int,
        ...
        self.stretch_target = float(ihs_target) if ihs_target is not None else float(stretch_target)
        self._cancel      = False
...
class SplitPreviewWidget(QWidget):
    ...
    def __init__(self, before_rgb: np.ndarray, parent=None):
        ...
        self._preview_btn.setStyleSheet(
            "QPushButton {"
            "  background: qlineargradient(x1:0, y1:0, x2:0, y2:1, stop:0 #2ec4b6, stop:1 #0fa3b1);"
            "  color: white;"
            "  font-weight: bold;"
            "  font-size: 9pt;"
            "  border: none;"
            ...
            "}"
            "QPushButton:hover {"
            "  background: qlineargradient(x1:0, y1:0, x2:0, y2:1, stop:0 #3cd5c7, stop:1 #1fb4c2);"
            "}"
            "QPushButton:pressed {"
            "  background: #0fa3b1;"
            "}"
    ...
    def paintEvent(self, ev):
        ...
        if self._split_active:
            ...
            glow_alpha = int(45 * reveal)
            core_alpha = int(230 * reveal)
            painter.setPen(QPen(QColor(46, 196, 182, glow_alpha), 6, Qt.PenStyle.SolidLine))
            painter.drawLine(split_x, 0, split_x, H)
            # Core line
            painter.setPen(QPen(QColor(46, 196, 182, core_alpha), 2, Qt.PenStyle.SolidLine))
            painter.drawLine(split_x, 0, split_x, H)

            # --- Handle pill (centre of divider) ---
            handle_h, handle_w = 54, 20
            hx = split_x - handle_w // 2
            hy = H // 2 - handle_h // 2
            handle_path = QPainterPath()
            handle_path.addRoundedRect(float(hx), float(hy),
                                       float(handle_w), float(handle_h), 10, 10)
            painter.setPen(Qt.PenStyle.NoPen)
            painter.setBrush(QBrush(QColor(15, 23, 30, int(220 * reveal))))
            painter.drawPath(handle_path)
            
            painter.setPen(QPen(QColor(46, 196, 182, int(210 * reveal)), 1.5))
            painter.setBrush(Qt.BrushStyle.NoBrush)
            painter.drawPath(handle_path)
            
            # Disegna 3 grip lines orizzontali al centro del pill per un look tattile/hardware
            painter.setPen(QPen(QColor(255, 255, 255, int(100 * reveal)), 1.5))
            painter.drawLine(split_x - 4, hy + handle_h // 2 - 4, split_x + 4, hy + handle_h // 2 - 4)
            painter.drawLine(split_x - 4, hy + handle_h // 2,     split_x + 4, hy + handle_h // 2)
            painter.drawLine(split_x - 4, hy + handle_h // 2 + 4, split_x + 4, hy + handle_h // 2 + 4)

            # --- Label badges con design Glassmorphic ---
            def _draw_badge(text, cx, cy, is_after=False):
                ...
                by = cy
                badge = QPainterPath()
                badge.addRoundedRect(float(bx), float(by), float(bw), float(bh), 5, 5)
                
                # Sfondo glassmorphic
                painter.setPen(QPen(QColor(255, 255, 255, int(35 * reveal)), 1))
                painter.setBrush(QBrush(QColor(10, 15, 22, int(180 * reveal))))
                painter.drawPath(badge)
                
                # Colore del testo
                if is_after:
                    # Colore teal brillante per l'after
                    painter.setPen(QColor(46, 196, 182, int(240 * reveal)))
                else:
                    # Colore grigio chiaro per il prima
                    painter.setPen(QColor(224, 231, 238, int(230 * reveal)))
                
                painter.drawText(bx + pad_x, by + pad_y + fm.ascent(), text)

            if split_x > 90:
                _draw_badge("BEFORE", ox + 10, oy + 10, is_after=False)
            if W - split_x > 90:
                _draw_badge("AFTER", split_x + 10, oy + 10, is_after=True)
        ...
        if self._selection_visible and self._draw_rect is not None:
            ...
            # Sfondo semitrasparente e bordo tratteggiato moderno per l'area
            painter.setPen(QPen(QColor(46, 196, 182, 100), 1, Qt.PenStyle.DashLine))
            painter.setBrush(QBrush(QColor(46, 196, 182, 15)))
            painter.drawRoundedRect(float(rx), float(ry), float(rw), float(rh), 6, 6)

            # Angoli spessi "Camera Rangefinder style" per precisione scientifica
            painter.setPen(QPen(QColor(46, 196, 182, 240), 3))
            painter.setBrush(Qt.BrushStyle.NoBrush)
            len_val = min(16.0, rw / 4.0)
            
            # Top-Left
            painter.drawLine(rx, ry + len_val, rx, ry)
            painter.drawLine(rx, ry, rx + len_val, ry)
            
            # Top-Right
            painter.drawLine(rx + rw - len_val, ry, rx + rw, ry)
            painter.drawLine(rx + rw, ry, rx + rw, ry + len_val)
            
            # Bottom-Left
            painter.drawLine(rx, ry + rh - len_val, rx, ry + rh)
            painter.drawLine(rx, ry + rh, rx + len_val, ry + rh)
            
            # Bottom-Right
            painter.drawLine(rx + rw - len_val, ry + rh, rx + rw, ry + rh)
            painter.drawLine(rx + rw, ry + rh - len_val, rx + rw, ry + rh)
        ...
        if abs(self._zoom - 1.0) > 0.01:
            ...
            zbg = QPainterPath()
            zbg.addRoundedRect(float(bx), float(by), float(bw), float(bh), 4, 4)
            
            # Zoom indicator glassmorphic
            painter.setPen(QPen(QColor(255, 255, 255, 30), 1))
            painter.setBrush(QBrush(QColor(10, 15, 22, 190)))
            painter.drawPath(zbg)
            painter.setPen(QColor(46, 196, 182, 230))
        ...
        if self._zoom > 1.05 and pm_w > 0 and pm_h > 0:
            ...
            # Viewport inside minimap
            painter.setPen(QPen(QColor(46, 196, 182, 230), 1.5))
            painter.setBrush(QBrush(QColor(46, 196, 182, 35)))
            painter.drawRoundedRect(float(rx), float(ry), float(rw), float(rh), 2, 2)
            
            # Minimap border (discreta, con doppio bordo per contrasto)
            painter.setPen(QPen(QColor(20, 25, 35, 200), 2))
            painter.setBrush(Qt.BrushStyle.NoBrush)
            painter.drawRect(mm_x, mm_y, mm_w, mm_h)
            painter.setPen(QPen(QColor(46, 196, 182, 100), 1))
            painter.drawRect(mm_x, mm_y, mm_w, mm_h)
        else:
            self._mm_rect = None
...
def _make_model_row(parent, label: str, path: Path, load_cb, remove_cb):
    """Returns (row_widget, status_label, btn_load, btn_remove) — status updates when path changes."""
    col = QWidget(parent)
    ...
    btn_load = QPushButton("Load...", col)
    btn_load.setFixedWidth(64)
    btn_load.setFixedHeight(24)
    btn_load.setStyleSheet("QPushButton { font-size: 9px; padding: 0px 5px; margin: 0px; min-height: 20px; font-weight: bold; background-color: #007aff; color: white; border-radius: 4px; } QPushButton:hover { background-color: #0a84ff; }")
    ...
    return col, status, btn_load, btn_r
...
class PrismGUI(QMainWindow):
    ...
    def __init__(self, siril, config: dict,
        ...
        # Debounce timer for stretch updates to make UI highly responsive
        self._stretch_timer = QTimer(self)
        self._stretch_timer.setSingleShot(True)
        self._stretch_timer.setInterval(120)  # 120 ms
        self._stretch_timer.timeout.connect(self._on_stretch_toggled_delayed)
    ...
    _DARK_STYLESHEET = """
        ...
        QComboBox:hover { border-color: #2ec4b6; }
        QComboBox:focus { border-color: #3cd5c7; }
        QComboBox::drop-down {
        ...
        QComboBox QAbstractItemView {
            background-color: #161d26;
            color: #e9eef5;
            selection-background-color: #0fa3b1;
        ...
        QSpinBox:hover, QDoubleSpinBox:hover { border-color: #2ec4b6; }
        QSpinBox:focus, QDoubleSpinBox:focus { border-color: #3cd5c7; }
        ...
        QPushButton:hover { background-color: #1c2632; border-color: #2ec4b6; }
        QPushButton:pressed { background-color: #2ec4b6; color: #071019; }
        ...
        QPushButton#primaryBtn:hover {
            background-color: qlineargradient(x1:0,y1:0,x2:1,y2:0,
                stop:0 #1fb4c2, stop:1 #3cd5c7);
        }
        QPushButton#primaryBtn:pressed { background-color: #0fa3b1; }
        QPushButton#primaryBtn:disabled { background-color: #26333f; color: #627280; }
        ...
        QPushButton#zoomBtn {
            ...
            padding: 4px 10px;
            font-size: 9pt;
            min-height: 20px; min-width: 26px;
            color: #dbe3ed;
        }
        QPushButton#zoomBtn:hover { border-color: #2ec4b6; color: #8fe8fa; }
    """
    ...
    def _build_ui(self):
        ...
        lbl_pro_badge = QLabel(" PRO ")
        lbl_pro_badge.setStyleSheet(
            "color: #2ec4b6; font-size: 8pt; font-weight: 800; "
            "border: 1.5px solid #2ec4b6; border-radius: 5px; padding: 1px 6px; "
            "background: rgba(46, 196, 182, 12);"
        )
        title_row.addWidget(lbl_pro_badge)
        ...
        self.lbl_weights_status.setStyleSheet("font-weight: bold; font-size: 11pt;")
        model_select_row.addWidget(self.lbl_weights_status)
        
        self.btn_get_models = QPushButton("Get free Prism models...")
        self.btn_get_models.setObjectName("zoomBtn")
        self.btn_get_models.setStyleSheet("padding: 4px 8px; font-size: 9pt;")
        self.btn_get_models.setToolTip("Opens https://syqon.eu/free-prism-mini to download Prism models")
        self.btn_get_models.clicked.connect(lambda: webbrowser.open(FREE_PRISM_MODELS_URL))
        model_select_row.addWidget(self.btn_get_models)
        
        self.btn_load_weights = QPushButton("Load...")
        self.btn_load_weights.setObjectName("zoomBtn")
        self.btn_load_weights.setStyleSheet("padding: 4px 8px; font-size: 9pt;")
        self.btn_load_weights.clicked.connect(self._browse_and_install_selected_model)
        model_select_row.addWidget(self.btn_load_weights)
        ...
        if not self.is_sequence:
            ...
            self.chk_show_stretch = QCheckBox("Show stretch in preview")
        ...
        if not self.is_sequence:
            stretch_col = QVBoxLayout()
            stretch_col.setSpacing(4)
            
            method_lbl = QLabel("Stretch Method")
            method_lbl.setStyleSheet("color: #dfe7f1; font-size: 9pt; font-weight: 600;")
            stretch_col.addWidget(method_lbl)
            
            self.combo_stretch_method = QComboBox()
            self.combo_stretch_method.addItem("Statistical (Auto)", "statistical")
            self.combo_stretch_method.addItem("IHS (Aggressive)", "ihs")
            self.combo_stretch_method.currentIndexChanged.connect(self._on_stretch_method_changed)
            stretch_col.addWidget(self.combo_stretch_method)
            
            stretch_lbl = QLabel("Target median")
            stretch_lbl.setStyleSheet("color: #dfe7f1; font-size: 9pt; font-weight: 600;")
            stretch_col.addWidget(stretch_lbl)
            
            self.spin_stretch_target = QDoubleSpinBox()
            self.spin_stretch_target.setRange(0.01, 0.50)
            self.spin_stretch_target.setSingleStep(0.01)
            self.spin_stretch_target.setDecimals(3)
            self.spin_stretch_target.setValue(0.25)
            self.spin_stretch_target.setToolTip(
                "Target median brightness for the temporary stretch.\n"
                "Lower values = more aggressive stretch.\n"
                "Higher values = gentler stretch."
            )
            stretch_col.addWidget(self.spin_stretch_target)
            
            stretch_hint = QLabel("Preview-only control")
            stretch_hint.setObjectName("sectionHint")
            stretch_col.addWidget(stretch_hint)
            ...
            stretch_col.addLayout(stretch_preset_row)
            second_adv_row.addLayout(stretch_col, 1)
        advanced_form.addLayout(second_adv_row)

        self.chk_amp = QCheckBox("AMP mixed precision (CUDA / ROCm / MPS)")
        advanced_form.addWidget(self.chk_amp)

        self.chk_no_gpu = QCheckBox("Force CPU (disable GPU acceleration)")
        self.chk_no_gpu.stateChanged.connect(self._on_gpu_toggle)
        advanced_form.addWidget(self.chk_no_gpu)

        try:
            dev_test = get_device(True)
            d_t = dev_test.type if isinstance(dev_test.type, str) else str(dev_test.type)
            if d_t == "cpu":
                if platform.system() == "Darwin" and platform.machine() != "arm64":
                    gpu_name = "Intel Mac CPU (Accelerated via vecLib/OpenMP)"
                else:
                    gpu_name = "CPU Only"
            elif d_t == "cuda":
                if torch.cuda.is_available():
                    gpu_name = f"NVIDIA / AMD ROCm ({torch.cuda.get_device_name(0)})"
                else:
                    gpu_name = "NVIDIA / AMD ROCm (CUDA)"
            elif d_t == "mps":
                mac_type = "Apple Silicon" if platform.machine() == "arm64" else "Intel Mac AMD GPU"
                gpu_name = f"{mac_type} (Metal MPS)"
            elif d_t == "xpu":
                gpu_name = "Intel GPU (XPU)"
            else:
                try:
                    import torch_directml
                    gpu_name = f"AMD / DirectML ({torch_directml.device_name(torch_directml.default_device())})"
                except Exception:
                    gpu_name = f"AMD / DirectML GPU ({dev_test})"
        except Exception:
            gpu_name = "Failed to detect Hardware"

        self.lbl_gpu_detected = QLabel(f"<b>Active Hardware:</b> {gpu_name}")
        self.lbl_gpu_detected.setStyleSheet("color: #30d158; font-size: 8.5pt; margin-top: 4px;")
        advanced_form.addWidget(self.lbl_gpu_detected)

        if not self.is_sequence:
            self.chk_show_stretch.toggled.connect(self._on_stretch_toggled)
            self.spin_stretch_target.valueChanged.connect(self._on_stretch_toggled)

        controls_layout.addWidget(advanced_box)

        # AI Model Weights Section (Model Management)
        self.mbox_models = QGroupBox("AI Model Weights")
        mform = QVBoxLayout(self.mbox_models)
        mform.setContentsMargins(14, 18, 14, 12)
        mform.setSpacing(8)

        lbl_subtitle = QLabel("Manage model files. Click 'Load...' next to a model to install its .pt file.")
        lbl_subtitle.setStyleSheet("font-size: 8.5pt; color: #8e8e93; font-style: italic; margin-bottom: 4px;")
        ...
        for key, label, path_fn in [
            ("mini", "Prism Mini", _mini_path),
            ("deep", "Prism Deep", _deep_path),
        ]:
            col_widget, slbl, btn_load, btn_r = _make_model_row(
                self.mbox_models, label, path_fn(),
                lambda checked=False, k=key: self._install_single_model(k),
                lambda checked=False, k=key: self._remove_model(k),
            )
            self._model_status[key] = slbl
            self._model_rows[key] = (col_widget, slbl, btn_load, btn_r)
        ...
        self.btn_get_free = QPushButton("Get Free Models")
        self.btn_get_free.setToolTip("Opens https://syqon.eu/free-prism-mini in your browser")
        self.btn_get_free.clicked.connect(lambda: webbrowser.open(FREE_PRISM_MODELS_URL))
        self.btn_get_free.setFixedHeight(26)
        self.btn_get_free.setStyleSheet("QPushButton { font-size: 8.5pt; padding: 2px 6px; min-height: 18px; font-weight: 600; }")
        actions_row.addWidget(self.btn_get_free, 1)

        self.btn_get_deep = QPushButton("Get Deep Prism")
        self.btn_get_deep.setToolTip("Opens https://syqon.eu/prism in your browser")
        self.btn_get_deep.clicked.connect(lambda: webbrowser.open(PRISM_DEEP_URL))
        self.btn_get_deep.setFixedHeight(26)
        self.btn_get_deep.setStyleSheet("QPushButton { font-size: 8.5pt; padding: 2px 6px; min-height: 18px; font-weight: 600; }")
        actions_row.addWidget(self.btn_get_deep, 1)

        btn_cc = QPushButton("Clear AI Cache")
        btn_cc.setFixedHeight(26)
        btn_cc.setStyleSheet("QPushButton { font-size: 8.5pt; padding: 2px 6px; min-height: 18px; font-weight: normal; }")
        btn_cc.clicked.connect(
            lambda: (clear_model_cache(), self.lbl_status.setText("AI cache cleared."))
        )
        actions_row.addWidget(btn_cc)
        ...
        lay_controls = QVBoxLayout(self.tab_controls)
        lay_controls.setContentsMargins(0, 6, 0, 0)
        lay_controls.setSpacing(10)
        lay_controls.addWidget(essentials_box)
        lay_controls.addWidget(advanced_box)
        lay_controls.addStretch()
        self.tabs.addTab(self.tab_controls, "Settings")

        # Tab 2: Models
        self.tab_models = QWidget()
        lay_models = QVBoxLayout(self.tab_models)
        lay_models.setContentsMargins(0, 6, 0, 0)
    ...
    def _load_config_into_ui(self):
        cfg = self.config
        model_val = cfg.get("model", "mini")
        idx = self.cmb_model.findData(model_val)
        if idx >= 0:
        ...
        if not self.is_sequence:
            method = cfg.get("stretch_method", "statistical")
            target = cfg.get("stretch_target", 0.25)
            if "ihs_target" in cfg and "stretch_target" not in cfg:
                target = cfg["ihs_target"]
                method = "ihs"
            
            idx = self.combo_stretch_method.findData(method)
            if idx >= 0:
                self.combo_stretch_method.setCurrentIndex(idx)
            self.spin_stretch_target.setValue(float(target))
        self._sync_model_ui()
    ...
    def _gather_config(self) -> dict:
        ...
        if not self.is_sequence:
            cfg["stretch_method"] = self.combo_stretch_method.currentData()
            cfg["stretch_target"] = float(self.spin_stretch_target.value())
            cfg["ihs_target"] = float(self.spin_stretch_target.value())
        else:
            cfg["stretch_method"] = "statistical"
            cfg["stretch_target"] = 0.25
            cfg["ihs_target"] = 0.25
        return cfg
    ...
    def _sync_model_ui(self):
        ...
        if is_deep:
            ...
            if exists:
                ...
                self.lbl_model_hint.setText(
                    "Prism Deep is selected but not installed yet. Click 'Get free Prism models...' or 'Load...' to import the model file."
                )
        else:
            exists = _mini_path().exists()
            if exists:
                self.lbl_model_hint.setText(
                    "Prism Mini is the fastest starting point for most images."
                )
            else:
                self.lbl_model_hint.setText(
                    "Prism Mini is selected but not installed yet. Click 'Get free Prism models...' or 'Load...' to import the model file."
                )
            
        if exists:
            self.lbl_weights_status.setText("✓")
            self.lbl_weights_status.setStyleSheet("color: #10B981; font-weight: bold; font-size: 12pt;")
            self.btn_get_models.hide()
            self.btn_load_weights.hide()
            if hasattr(self, 'btn_run') and self.btn_run is not None:
                self.btn_run.setEnabled(True)
        else:
            self.lbl_weights_status.setText("✗")
            self.lbl_weights_status.setStyleSheet("color: #EF4444; font-weight: bold; font-size: 12pt;")
            self.btn_get_models.show()
            self.btn_load_weights.show()
            if hasattr(self, 'btn_run') and self.btn_run is not None:
    ...
    def _refresh_model_status(self):
        ...
        for key, p in paths.items():
            if hasattr(self, '_model_status') and key in self._model_status:
                installed = p.exists() and p.is_file()
                color = '#30d158' if installed else '#ff453a'
                status_text = 'Installed' if installed else 'Missing'
                self._model_status[key].setText(
                    f"<span style='color:{color}; font-size: 11px;'>●</span> <b>{labels[key]}:</b> <span style='color:{color}; font-size: 11px;'>{status_text}</span>"
                )
                if hasattr(self, '_model_rows') and key in self._model_rows:
                    _, _, btn_load, btn_r = self._model_rows[key]
                    if installed:
                        btn_load.hide()
                        btn_r.show()
                    else:
    ...
    def _install_single_model(self, key: str):
        expected_name = "prism_mini.pt" if key == "mini" else "prism_deep.pt"
        label = "Prism Mini" if key == "mini" else "Prism Deep"

        from PySide6.QtWidgets import QFileDialog
        src, _ = QFileDialog.getOpenFileName(
            self,
            f"Select {label} model file ({expected_name})",
            "",
        ...
        if src_path.name.lower() != expected_name and not (key in src_path.name.lower()):
            QMessageBox.warning(
                self, "SyQon Prism",
                f"Incorrect file selected.\n\n"
                f"Expected filename: {expected_name}\n"
                f"Selected filename: {src_path.name}\n\n"
                "Please select the correct model file."
            )
            return

        try:
            self.engine_dir.mkdir(parents=True, exist_ok=True)
            dst = self.engine_dir / expected_name
            if dst.resolve() != src_path.resolve():
                import shutil
                self.lbl_status.setText("Installing… please wait.")
                QApplication.processEvents()
                shutil.copy2(str(src_path), str(dst))
            self.lbl_status.setText(f"✓ {label} installed successfully.")
            QMessageBox.information(
                self, "SyQon Prism",
                f"{label} installed successfully.\n\nLocation: {dst}"
            )
            clear_model_cache()
    ...
    def _remove_model(self, key: str):
        ...
        try:
            ...
            clear_model_cache()
            self.lbl_status.setText(f"Removed {p.name}.")
        except Exception as e:
            QMessageBox.warning(self, "Prism", f"Remove failed: {e}")
        self._refresh_model_status()
    ...
    def _apply_stretch_preset(self, value: float):
        if not hasattr(self, 'spin_stretch_target') or self.spin_stretch_target is None:
            return
        self.spin_stretch_target.setValue(float(value))
        self.lbl_status.setText(f"Stretch preset set to {value:.2f}.")

    def _browse_and_install_selected_model(self):
        current = self.cmb_model.currentData()
        expected_name = "prism_mini.pt" if current == "mini" else "prism_deep.pt"
        
        from PySide6.QtWidgets import QFileDialog
        src, _ = QFileDialog.getOpenFileName(
            self,
            f"Select {expected_name} model file",
            "",
            "PyTorch model (*.pt);;All Files (*)",
        )
        if not src:
        ...
        try:
            ...
            QMessageBox.information(
                self, "SyQon Prism",
                f"Model installed successfully.\n\nLocation: {dst}"
            )
            self._refresh_model_status()
        except Exception as exc:
            QMessageBox.critical(self, "SyQon Prism", f"Failed to install model:\n{exc}")
    ...
    def _on_stretch_toggled(self, *_):
        """Start the debounce timer to avoid freezing the UI on rapid changes."""
    ...
    def _on_stretch_toggled_delayed(self):
        """Immediately update the preview when the stretch debounce timer fires."""
        if self.chk_show_stretch.isChecked():
            method = self.combo_stretch_method.currentData() if hasattr(self, 'combo_stretch_method') else "statistical"
            target = float(self.spin_stretch_target.value())
            params = compute_stretch_params(self.before_rgb, method, target)
            stretched = apply_stretch(self.before_rgb, method, params, target)
    ...
    def _on_stretch_method_changed(self):
        method = self.combo_stretch_method.currentData()
        if method == "statistical":
            self.spin_stretch_target.setValue(0.25)
        elif method == "ihs":
            self.spin_stretch_target.setValue(0.40)
        self._on_stretch_toggled()
    ...
    def _on_preview_tile(self):
        ...
        method = cfg.get("stretch_method", "statistical")
        target = float(cfg.get("stretch_target", 0.25))
        stretch_params = compute_stretch_params(crop, method, target)
        full_before = self.before_rgb.astype(np.float32, copy=True)
        before_display = (
            apply_stretch(crop, method, stretch_params, target)
            if self.chk_show_stretch is not None and self.chk_show_stretch.isChecked()
            else crop
        )

        if self.chk_show_stretch is not None and self.chk_show_stretch.isChecked():
            full_before_display = apply_stretch(full_before, method, compute_stretch_params(full_before, method, target), target)
        else:
        ...
        try:
            ...
            after_display = apply_stretch(result[..., :3], method, stretch_params, target)
    ...
    def _on_run(self):
        ...
        if not self.is_sequence:
            ...
            # Optionally build the stretched version of the padded image.
            if (self.chk_show_stretch is not None
                    and self.chk_show_stretch.isChecked()):
                stretch_params_padded = compute_stretch_params(padded_linear, method, target)
                padded_before_for_preview = apply_stretch(padded_linear, method, stretch_params_padded, target)
            else:
        ...
        self._worker = PrismWorker(
            ...
            stretch_method= cfg.get("stretch_method", "statistical"),
            stretch_target= cfg.get("stretch_target", 0.25),
        )
    ...
    def _on_finished_ok(self, denoised_for_siril, denoised_padded_rgb):
        ...
        if not self.is_sequence:
            ...
            show_stretch = (self.chk_show_stretch is not None
                            and self.chk_show_stretch.isChecked())
            if show_stretch:
                before_preview = self._padded_preview
                params_for_preview = compute_stretch_params(self._padded_linear, method, target)
                after_preview = apply_stretch(denoised_padded_rgb, method, params_for_preview, target)
            else:
                before_preview = self._padded_linear
                after_preview = denoised_padded_rgb
            
            self.preview.reset_before(before_preview)
            self.preview.reset_after(after_preview)
    ...
    for idx, frame_num in enumerate(included):
        ...
        try:
            ...
            denoised_padded = process_image(
                ...
                progress_callback = _prog,
                stretch_method    = config.get("stretch_method", "statistical"),
                stretch_target    = float(config.get("stretch_target", 0.25)),
            )
            print()
...
def main():
    ...
    parser.add_argument("--stretch-method", type=str, default=None, choices=["statistical", "ihs"])
    parser.add_argument("--stretch-target", type=float, default=None)
    ...
    if args.stretch_method is not None: config["stretch_method"] = args.stretch_method
    if args.stretch_target is not None: config["stretch_target"] = args.stretch_target
