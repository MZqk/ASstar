...
主流程是 starless-first 后期链路：输入统一和裁切 -> 输入线性状态判定 -> 冻结主目标/策略与处理计划 -> 条件背景提取、自动设备几何/platesolve、SPCC 优先物理校色（异常时宽带正常回退 PCC；无可用 Gaia 时宽带可使用自动局部参考的非物理降级色彩；双窄带 PCC 仅作强制复核的降级基础校色；Stage 4 HOO 艺术映射保持隔离）、噪声模型驱动线性反卷积/降噪 -> 线性去星与确定性星色修复 -> Starless 主体拉伸和本地曲线/蒙版增强（可显式启用确认 Ha/OIII 的目标首选伪色） -> 受控回星 -> 最终降噪和科学/展示/编辑版导出。输入状态只接受可追溯证据，分为 `linear / nonlinear / unknown`；非线性或未知输入只执行安全裁切并进入复核导出，不运行破坏性的线性处理、去星或 Starless 增强。
...
Stage 7 的 Stage 6 linked preview 仍只是屏幕显示参考。默认 `stage7_rendition_intent=vivid_safe` 与 `auto_display90` 会在 A/B 外生成同一 GUI D 曲线的 `cand_display70/82/90` 梯度，并在冻结的 Stage 6 主体/背景 ROI 上统一测量可见度、主体跨度、饱和度中位数、微对比度和背景安全余量；有余量且主体色彩不足时，再生成只增强主体低频色度、保持 Rec.709 亮度并逐像素限制 RGB headroom 的 `cand_vivid_safe`。v6 排序先过滤数值、文件/形状、真正新增裁切、核心和结构技术硬门，并继续保留主体亮度硬下限；可靠主体仍使用既有亮度优先分支，低 lift 星云则以可见度 `25%`、主体跨度 `30%`、饱和度 `15%`、微对比 `30%` 形成呈现质量，再按呈现 `70%` + 安全余量 `30%` 排序，亮度目标只作末级同分裁决。Display GUI 参考色度比值仍须 `<=1.05`，绝对硬上限为 `0.30`；全任务“放松/无限”档不缩放 Stage 7 的裁切、结构、色度安全门。所有候选出现硬画质失败时 `formal_accepted=false`：兼容字段 `stage7_forced_delivery_enabled` 只允许保留最佳失败候选作为 `delivery_class=review_only` 诊断证据，Stage 8 从冻结 Stage 6 含星输入生成安全预览，Stage 9 不执行无星重混，Stage 10 只输出 `result_review*`。

Stage 4 未接受 SPCC/PCC 物理校色时，Stage 7 会在排除主体、星云及可识别银河盘面信号后独立检查背景绝对色偏，Stage 10 对最终候选复测；`background_chroma_load` 默认超过 `0.12` 即要求复核。无网络且没有本地 Gaia 时，只有确认线性的宽带 RGB/OSC 可从 `stage4_pre_pcc.fit` 运行“自动局部参考”：安全背景样点被确定性拆成估计集和留出集，候选只削减偏高通道并通过独立质量门；恒星集合伪白参考首发只生成影子证据。结果固定为非物理色彩并强制 `result_review*`，不能称为 SPCC/PCC。单色、非线性、未知通道和窄带继续保色。
...
3. 应用只显示一个自动处理计划，不提供手动阶段选择。只有已验签 `run-manifest.json`、`checkpoint-manifest.json`、v2 阶段契约、累计配置指纹和产物 SHA-256 全部匹配的产品任务，才可能从 Stage 1、2 或 5 后继续；普通目录和 `result_linear.fit` 文件名本身不能证明断点。macOS 的“Starun > 设置…”（`Command-,`）会在主窗口上方居中打开紧凑、不可缩放的独立窗口，维护联网、保留中间文件、完成后断点收敛、输出、正式/复核和计算设备等应用默认值；工具栏“任务选项”和任务页“处理参数…”只处理当前任务。处理参数顶部“通用配置”保存输出、审查、计算设备、自动调参、重试上限/间隔、复核包、受管理输出和 `checkpoint_mode`，并与应用默认值同步及冻结进任务清单；其后提供只缩放 Stage 2–9 已登记数值门禁的全任务“门禁策略”，下方按 Stage 1–10 单开分组，首次默认展开 Stage 2。Stage 4 普通配置“无 Gaia 时”可选择“自动局部参考（非物理色彩，需复核）”或“保留输入颜色（需复核）”；两者都允许离线任务继续，后者不会运行旧的局部星点恢复。默认档位使用代码中的静态门禁默认值；放松模式将显式登记的数值验收阈值按宽严方向放宽 `3×`，无限模式放宽 `10×` 并临时强制本轮输出待复核。Stage 1、Stage 10 和最终交付安全门不参与放宽；布尔门、算法目标、采样/重试设置和结构性安全检查也不参与缩放。专家模式中的登记门禁会只读显示档位派生后的真实值；取消“跟随档位”后可在原专家安全范围内逐项覆盖，且专家覆盖优先。档位和 Stage 1–10 覆盖仅属于当前输入，切换输入时恢复默认档位与自动参数；通用配置继续保留。批量任务在开始时冻结同一份签名快照，运行期间设置窗口保持可查看但禁止修改；同一新队列按“源文件指纹＋完整规范参数哈希”跳过完全重复项，不同参数和手动重新运行不去重。首个 `online_unverified` Gaia SPCC 超时后，本队列余下任务会跳过在线 SPCC，但继续允许 localgaia、PCC 和离线降级；新队列自动复位。Stage 1/2/5 断点分别禁用已完成的 Stage 1、Stage 1–2、Stage 1–5，Stage 10 始终可配置。Stage 2/3/4/6 可显式选择安全保留模式，仍生成规范阶段产物和诊断，不改变 Stage 1–10 顺序。
   专家项按“执行策略 / 算法参数 / 过程门禁 / 质量验收 / 回退与失败”折叠显示；打开专家参数时优先展开执行策略。Stage 2–10 均可选择失败处置：`自动安全回退` 继续原候选链，`恢复输入并复核` 在决定性失败后停止搜索并回载本阶段输入，`写诊断后终止任务` 在阶段报告落盘后严格停止。Stage 10 的恢复来源只能是已验证 Stage 9 含星源；没有“忽略门禁并强制接受”，必须含星、最终质量报告以及相关性、通量和核心裁剪等交付不变量始终不可关闭。
   各阶段还可限制实际执行后端或子步骤：Stage 1 可调整配准失败率的降级线；Stage 2 第一方场旋覆盖检测与显式基础裁边，Stage 3 GraXpert/内置背景链，Stage 4 PCC/窄带降级/Header 引导解析，Stage 5 降噪后端与保留线性结果，Stage 6 SyQon/SASP，Stage 7 A/B/Display90 拉伸候选与 Display90 强度，Stage 8 `auto/limited/background_only/preserve` 强度上限与五个增强开关，Stage 9 可信含星保留、可调 PSF 恢复软目标及回星重试阶梯，Stage 10 保留源、末端降噪/饱和度/质量修复开关与后端策略。确认 Ha/OIII 的双窄带任务可在 Stage 8“哈勃色方案”中保持“自动（按目标类型）”，或手动指定 `HSO/SHO/OSH/OHS/HOS/HOO`；手动方案只改变艺术色盘，不改变 Stage 4 的 `R→Ha、G/B→OIII` 物理通道结论，也不能绕过 Starless、full 路由和画质门。用户模式只能收紧自动安全路由，不能把上游的 limited/skip 强制升级为完整处理。
...
- `Starun/<task-id>/runs/<run-id>/run-manifest.json`：验签的来源、断点指纹、当前新写的 `starun.processing-parameters.v5` 任务参数快照及逐项门禁档位审计；历史 v4 仅在验签后兼容读取并执行明确迁移，v1–v3、未知 schema、扁平载荷和未知字段在预检时报错，专家越界值与档位物理截断都会记录
...
- `stage4_auto_local_reference.json`：Stage 4 自动背景/恒星样本、fit/holdout 指标、候选增益、独立质量门、影子比较与检查点回滚证据；出现自动参考结果时固定披露“非物理色彩、需复核”
...
| `STARUN_STAGE7_*` | 阶段 6 冻结质量阈值和事务式修复控制；SyQon 输入固定保持线性且不做质量参数盲扫；字段名按当前 v5 参数注册表固定，历史 v4 仅兼容迁移 |
...
| `STARUN_STAGE4_NBN_*` / `STARUN_STAGE4_FILTER_HINT` | Stage 4 单次生成 `stage4_channel_mapping.json`；双窄带 HOO 归一化只消费该契约并生成 `stage4_hoo_artistic.fit` 艺术派生图，后续主链恢复 `stage4_physical_color.fit`，单色和不确认映射保色 |
...
| `STARUN_ABERRATION_API_ENABLE` / `STARUN_ABERRATION_PROVIDER` | Stage 5/10 SASP Aberration API fallback；默认关闭 API，provider 可设 `cpu` |
...
Stage 4 在确认线性的宽带 RGB/OSC 和双窄带输入上保存不可变的 `stage4_pre_pcc.fit`，然后在能力允许时以独立 Siril CLI 进程执行一次 Gaia DR3 SPCC。在 platesolve 或任何像素颜色操作前，它只生成一次 `stage4_channel_mapping.json`，不读取 `OBJECT` 猜滤镜；即使选择 `preserve` 也会写出该契约。大小写不敏感的 `FILTER` 可识别时拥有权威性，其他滤镜字段只作为 ignored evidence；`FILTER` 缺失或完全未知时才回退到 `FILTER1/FILTER2/INSFLNAM/FILTERNAME/FILTNAME/FILTNAM`，补充字段冲突则保持 `unknown`。默认使用在线 `-catalog=gaia`；显式关闭互联网时只使用预检通过的本地 `-catalog=localgaia`。本地 astrometric 与全部 48 个 XP 分块可用时保留 SPCC→PCC；只有 astrometric 时跳过 SPCC、保留 PCC；没有 astrometric 时跳过 platesolve/SPCC/PCC，并按 `stage4_offline_fallback_mode` 进入非物理自动局部参考或严格保色，预检状态为 `degraded_allowed` 而非阻断。Seestar S30/S30 Pro/S50 与 DWARF 2/3/mini 的远摄天文帧会从 FITS 设备身份选择对应的 Siril 传感器、滤镜和几何；Header 中的焦距/像元仍优先，显式环境覆盖优先于自动画像。双窄带按设备身份与滤镜联合识别：Seestar 的 `LP`/`LP_Starless`，以及 DWARF 3/mini 的 `Duo-Band`/`Dual-Band`，无需 Header 携带波长、FWHM 或发射线名称；实际 Hα/OIII 参数来自对应设备画像。其他厂家明确写入 `Dual-Band`/`Duo-Band` 时使用可审计的较低置信度通用路径；裸 `LP/LP_Starless` 同样以 `0.86` 进入宽松 HOO 路径，默认超过 `0.85` 门限，可通过提高该门限关闭。`IRCUT/IR Cut/UV-IR/Astro/VIS/Clear/No LP/LP Off` 明确按宽带处理。当前种子不含这些设备的广角 IMX586/IMX307/OS02K10 响应，识别为广角光路时会拒绝套用远摄画像。无法确认设备时，报告只显示 Header 提供的 `header_derived` 身份，Header 也没有身份时显示 `unknown`，不会回退成 Seestar/IMX585；SPCC 传感器无法精确解析时保持 fail-closed 并先回载检查点，随后宽带进入常规 PCC 异常回退，双窄带进入非物理的 PCC 降级基础校色并强制复核。Siril 的“解不精确”提示作为风险证据交给目标感知像素质量门，不再单独否决候选；SPCC 超时、命令失败或像素质量门拒绝时同样先回载不可变检查点。

双窄带物理校色、降级校色和艺术映射是三种明确语义：只有接受的 SPCC 物理结果保存为 `stage4_physical_color.fit`；SPCC 失败后通过质量门的 PCC 使用 `PCC_NARROWBAND_DEGRADED*` 方法名，只提供基础颜色矫正，不声明 `physical_color.accepted`，并固定 `requires_review=true`。HOO 归一化从本轮已选择的主颜色父本生成可选 `stage4_hoo_artistic.fit`，报告中固定 `feeds_main_pipeline=false`，随后重新载入 SPCC/PCC/保色父本再保存 `stage4_color.fit`。因此 HOO 艺术增益不会进入 Stage 5；降级 PCC 也只能走 Stage 10 的 `result_review*`。
