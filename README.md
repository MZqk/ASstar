...
- `Starun/<task-id>/runs/<run-id>/run-manifest.json`：验签的来源、断点指纹、当前新写的 `starun.processing-parameters.v6` 任务参数快照及逐项门禁档位审计；历史 v4/v5 仅在验签后兼容读取并移除已废弃的 Stage 3 经验门字段，v1–v3、未知 schema、扁平载荷和未知字段在预检时报错，专家越界值与档位物理截断都会记录
