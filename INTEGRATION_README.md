...
- 在线 SPCC/PCC 受 `STARUN_NETWORK_MODE` 总闸控制，GUI 默认在线；用户显式关闭联网后才强制本地 Gaia。首个 `online_unverified`、`catalog:gaia` SPCC 超时后，应用会话按 Siril 可执行文件/版本与 XP 端点证据指纹缓存 `operational_timeout_cached`；缓存键必须在 Siril 启动和网络能力预检完成后、pipeline worker 启动前计算。同一能力指纹的后续队列直接跳过在线 SPCC，但继续允许 localgaia、PCC 与离线降级。受影响任务在 worker 启动前显示持久警告，明确本轮不会重试 SPCC、Stage 4 与正式交付资格预期不会改善；队列仍自动继续生成 review-only fallback。Stage 4 尝试报告写 `expected_stage4_improvement=false`、`fallback_route=physical_pcc_only` 和刷新能力后再试的建议。Siril/端点证据变化会生成新指纹并重新探测；旧的单队列 circuit 仍只约束当前批次。
