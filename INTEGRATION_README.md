...
- `stage3_gate_profile` 的缺省值是 `output_first`，冻结配置缺少该字段时也使用此值；另有 `balanced` 和 `strict`。v5 `background_quality_report.json` 记录真实多尺度排除掩膜、统一 `reason_code`、GraXpert provenance，以及 `severity/warnings/hard_issues/profile/effective_thresholds`。输出优先与平衡档把一般 `3σ` 变化、改善不足、普通背景恶化和轻中度目标/图案变化降为软告警；软告警候选仍可保存并参与降权排序，只有档位定义的极端异常、命令失败、像素未变化、尺寸错误或非有限像素才硬拒绝。无软告警候选优先，同级按残余跨度显著性、绝对通量偏差、形态损失、质心、结构和图案噪声执行 Pareto 密集秩求和，再以背景分数和默认链顺序确定性解平局。全部候选硬拒绝或最终保存复检硬拒绝时回滚 `stage3_bg_input`、保存规范 `stage3_bgremoved`、标记 `degraded/review_required` 并继续；软告警复检不回滚。算法契约为 1.3.0。
