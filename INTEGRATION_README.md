...
- 本轮 Stage 7 可精确重放的 Display、`cand_b` linked-MTF 与 composite 候选只发布 `starun.stage7-matched-domain-transfer.v3`：合同认证实际 winner、父级 LUT/摘要和 linked gamma 的有序步骤；Stage 9 对原始含星图与 Starless 精确重放同一整链。无法序列化真实 winner 时合同为 `unavailable`，不得借用 `cand_b` 锚点。v1/v2 只为已存在的旧任务兼容读取；v3 的 schema、步骤、参数或摘要不一致时 fail-closed，不允许退回另一条 MTF/LUT。
...
- Stage 8 双窄带哈勃艺术分支默认开启，由 `STARUN_STAGE8_DUALBAND_PALETTE_ENABLE=0` 显式关闭；`STARUN_OPTIONAL_COLOR_TRANSFORM` 仅保留宽带/Stage 9 可选插件授权。宽带主体增色与双窄带 palette 互斥：前者只在 Stage 8 `auto/full`、Stage 7/结构质量已验收、无外部覆盖且通用 saturation 预算已预留时运行；确认 Ha/OIII 双窄带跳过主体增色，由 palette 独占艺术颜色预算。主体事务使用迁移后的低频/faint-signal 算法和 `1 + (legacy_factor - 1) × clamp(effective_saturation/0.40, 0, 1)` 系数，报告写入 `stage8_subject_chroma_report.json`；质量门拒绝保留结构基线，不补跑第二次通用增色。
- 任务参数 `stage8_dualband_palette_selection` 默认为 `auto`，也可手动指定 `HSO/SHO/OSH/OHS/HOS/HOO`；手动值是唯一候选，拒绝后回滚而不切换色盘。处理计划一次冻结 `requested_palette / automatic_palette / palette / selection_mode / manual_override`，Stage 8 只消费该结论。该分支只读取 Stage 4/Stage 5 验签恢复的冻结映射，不得使用当前 Header 重新分类；契约、映射、色盘结论缺失或字段异常时明确跳过。双窄带 SPCC 失败后，通过安全门的降级 PCC 父本仍执行伪色映射，但不清除 `requires_review`。自动模式从冻结目标首选开始，在六种固定 `Classic` 色盘中执行受门控竞争，并按主体/背景色度分离、主体色度增益、亮度漂移、裁剪增长和稳定顺序确定性选择；通道公式为 `H=R`、`O=(G+B)/2`、`S*=(H+O)/2`，报告披露 `S*` 为合成代理。调色前必须保存 `stage8_pre_palette.fit`；接受后更新规范 `stage8_enhanced.fit` 并进入 Stage 9，拒绝或异常精确回滚，回滚失败则降级并要求人工复核。Stage 9 禁止再运行 Vectra 二次转色。
...
- Stage 9 同域映射以 `stage7_stretch_quality.json.matched_domain_transfer` 为权威合同。任一 `cand_display70/82/90` 胜出时都必须重建并验签对应 LUT；需要额外亮核 tone 的候选使用验签的 v3 Display LUT + linked-luminance power 链，Stage 9 精确重放同一链。合同缺失、损坏或 SHA-256 不符时停用 Unscreen 并保留 Screen 基线，禁止改用 cand_b MTF。非 Display tone 候选使用下一条的 `closed_form_mtf_reference`。
