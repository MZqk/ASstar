...
| `pipeline/processing_parameters.py` | 当前 v5 参数注册表、历史 v4 兼容迁移、序列化、枚举与安全范围校验 |
...
- 每轮确保 `~/.local/share/siril-scripts/`，校验并同步最小 SPCC 元数据种子；元数据不可用时在启动 Siril 前禁用 SPCC，使宽带进入 PCC 异常回退、双窄带进入需复核的 PCC 降级基础校色。该种子覆盖 Seestar S30/S30 Pro/S50 与 DWARF 2/3/mini 所需的官方 Siril 传感器响应、UV/IR/Seestar LP/DWARF mini Astro 滤镜和白参考曲线，不含 Gaia XP 光谱。
- 完全离线的 platesolve/PCC 使用 runtime 下大小精确匹配受支持版本的 `siril_cat_healpix8_astro.dat`；离线 SPCC 使用独立的 `siril_cat1_healpix8_xpsamp/`，预检要求编号 `0–47` 的 48 个有效分块全部存在。Worker 将两条路径写入临时 Siril config，并分别注入 `STARUN_GAIA_ASTRO_CATALOG` / `STARUN_GAIA_PHOTO_CATALOG`。GUI 下载器目前只安装约 1.52 GB 的 astrometric/PCC 目录；项目、App 与离线资源包均不得携带任一 Gaia 目录。Gaia 缺失不再等同于核心资源失败：有 astrometric、无 XP 时路由到 PCC-only；无 astrometric 时按本轮 `stage4_offline_fallback_mode` 路由到 `auto_local_reference` 或 `preserve_input`，状态为 `degraded_allowed`。Siril、配置模板或流水线资源缺失仍然阻断。
...
| `STARUN_STAGE4_SPCC_ONLINE_UNVERIFIED_TIMEOUT_SEC` | `90` | 在线 XP 仅有端点可达证据时的单次 SPCC 上限，安全范围 30–180 秒；实际取它与普通 SPCC 超时的较小值，完整 `localgaia` 不受此附加上限影响 |
| `STARUN_STAGE4_OFFLINE_FALLBACK_MODE` | `auto_local_reference` | 无 astrometric Gaia 时选择非物理自动局部参考或 `preserve` 保色继续；两者均强制复核 |
| `STARUN_STAGE4_AUTO_REFERENCE_GLOBAL_WHITE_ENABLE` | `0` | 恒星集合伪白参考默认只生成影子报告；设为 `1` 后仍须通过独立对象、留出集和像素质量门 |
...
- Stage 3 获得执行授权后的默认链为“目标感知内置 Polynomial/RBF → 复合 Polynomial→RBF → GraXpert → ADBE → DBE → AutoDBE”。同一阶段先评估全部候选并排序；只有当前阶段出现无软告警且足够干净的候选才停止升级。`builtin_only` 保留内置和复合候选，`graxpert_only` 只运行 GraXpert；`stage3_plugin_fallback_enabled=False` 同时排除 GraXpert 和 ADBE 系列。Siril 内置候选继续使用经过源掩膜、覆盖掩膜、间距和空间覆盖审计的自定义样点，所有 `subsky` 命令必须带 `-existing`，拟合集与至少 4 个留出点保持分离。
- `stage3_gate_profile` 的缺省值是 `output_first`，冻结配置缺少该字段时也使用此值；另有 `balanced` 和 `strict`。v4 `background_quality_report.json` 在保持布尔返回接口的同时记录 `severity/warnings/hard_issues/profile/effective_thresholds`。输出优先与平衡档把一般 `3σ` 变化、改善不足、普通背景恶化和轻中度目标/图案变化降为软告警；软告警候选仍可保存并参与降权排序，只有档位定义的极端异常、命令失败、像素未变化、尺寸错误或非有限像素才硬拒绝。无软告警候选优先，同级按残余跨度显著性、绝对通量偏差、形态损失、质心、结构和图案噪声执行 Pareto 密集秩求和，再以背景分数和默认链顺序确定性解平局。全部候选硬拒绝或最终保存复检硬拒绝时回滚 `stage3_bg_input`、保存规范 `stage3_bgremoved`、标记 `degraded/review_required` 并继续；软告警复检不回滚。算法契约为 1.2.0。
...
- Stage 4 公共入口必须在 platesolve、SPCC/PCC 和 HOO 像素处理前只调用一次统一窄带解析器，即使 `preserve` 也写出 `stage4_channel_mapping.json`。解析器将大小写不敏感的 `FILTER` 作为权威字段；值可分类时其他字段仅记为 ignored evidence，缺失或完全未知时才回退 `FILTER1/FILTER2/INSFLNAM/FILTERNAME/FILTNAME/FILTNAM`，且不读取 `OBJECT`。同一契约对象必须写入 `channel_profile.narrowband_mapping`、`color_calibration_report.channel_mapping`，并传给通道语义、SPCC 窄带参数和 HOO 归一化。受控设备为 Seestar S30/S30 Pro/S50 的 `LP/LP_Starless` 与 DWARF 3/mini 的 `Duo-Band/Dual-Band`；裸 `LP/LP_Starless` 为 `0.86 / authoritative_filter_field_hint`，默认通过 `0.85` 门限，可通过提高门限关闭；DWARF 2 没有虚构的内置双窄带画像，但可走通用滤镜文字路径。
- Stage 4 必须先保存不可变的 `stage4_pre_pcc.fit`。宽带与冻结契约确认 Ha/OIII 映射的双窄带都优先通过独立 Siril CLI 执行一次 Gaia DR3 SPCC；默认请求在线 Gaia，显式关闭联网时物理命令只使用已验证的本地 Gaia，不存在的命令由能力决策直接跳过。XP 在线端点可达仅代表 `online_unverified`，SPCC 单次预算默认最多 90 秒；完整 `localgaia` 为 `local_verified`，继续使用普通最高 300 秒配置。候选解不精确、命令失败或质量门拒绝后，必须逐像素验证检查点或内存基线精确恢复，再从同一基线执行一次最长 180 秒的 PCC；双重恢复失败时阻断主输出，禁止从 SPCC 候选串联到 PCC。
- 双窄带只有接受的 SPCC 结果保存为 `stage4_physical_color.fit`。SPCC 不可用时，PCC 候选通过同一像素安全门后可作为 `PCC_NARROWBAND_DEGRADED*` 基础颜色矫正进入主链，但报告必须同时写入 `physical_color.accepted=false`、`degraded_color_correction.applied=true` 和 `requires_review=true`。HOO 归一化只写隔离的 `stage4_hoo_artistic.fit`，报告必须为 `feeds_main_pipeline=false`；保存后重新加载已选择的 SPCC/PCC/保色父本再写 `stage4_color.fit`，不得用降级 PCC 或 HOO 艺术增益冒充物理校色。
- 无可用 astrometric Gaia 时，Stage 4 不调用 platesolve/SPCC/PCC，`stage4_psolved.fit` 只是输入镜像。仅 `linear + broadband_rgb_osc + auto_local_reference` 可从不可变检查点运行纯 NumPy 自动局部参考：背景样点必须独立 fit/holdout，候选只削减偏高通道，并执行中位数、P90、改善点比例与空间色度梯度留出门；A 拒绝时 B 不检测星群。A 接受后恒星集合最多以 256 个非颜色排序、空间均衡对象做伪白参考影子评估。物理 SPCC/PCC 已通过时完全跳过 A/B，只写轻量报告。独立质量门通过后仍固定 `physical_color.accepted=false`、`degraded_color_correction.applied=true`、`requires_review=true`。`preserve` 路由会跳过新候选和旧局部星点恢复并原样保色。
- 新自动候选拒绝或写入/保存失败后必须验证像素精确恢复 `stage4_pre_pcc`，再允许现有星点软掩膜局部恢复；任何恢复失败都阻断主输出。自动局部参考、旧局部恢复或保色均必须进入 `pipeline-result.json` 的全局 `review_required`，让 Stage 10 只使用 `result_review*` 正式命名门。单色、非线性、未知通道和窄带不执行自动全局参考。
...
- Stage 7 参数模式仍只有 `auto/manual`；任一签名 Asinh/GHS override 把 `auto` 规范化为 `manual`。默认 `stage7_rendition_intent=vivid_safe` 在同一冻结主体/背景 ROI 上测量 preview 与候选的可见度、主体跨度、饱和度中位数/P95 和微对比度。v4 `hard_gate_bounded_presentation_score` 先过滤技术硬门，再按目标类型计算封顶呈现效用 70% + 背景色度/颜色方向/新增裁切安全余量 30%；饱和度超过目标不继续加分。星云、星系和星团分别偏重鲜艳度、自然层次和星色/核心保护。全任务 relaxed/unlimited 只缩放 Stage 7 可见度/P50 等呈现门，不缩放裁切、结构、LUT/MTF、背景色度和颜色方向安全门。
- Stage 7 默认 `auto_display90` 仅在 Stage 6 Starless 为 `accepted` 且参数模式为 `auto` 时，在 A/B 外从同一 GUI linked D 合同生成 `stage7_cand_display70/82/90.fit`；默认高档 `0.90`，中/低档为 `-0.08/-0.20`，各自认证 65536 点共享 RGB LUT。`balanced` 只保留 D82，`conservative` 只保留 D70；vivid-safe 可按需在已验收 tone 父本上生成 `stage7_cand_vivid_safe.fit`，只增强冻结主体宽尺度色度、保持 Rec.709 亮度并逐像素限制到 `0.995` headroom。`auto_dual` 保持 A/B 基础语义；`manual + display90_only` 仍拒绝。
...
- Stage 7 背景色度救援不再固定为 `0.10/0.20/0.35`，而是按当前 `chroma_load` 与有效绝对目标反解最多三档，默认上限 `0.90`，每档都从冻结源重放并重跑完整门禁。若 `auto_fallback` 耗尽且 `stage7_forced_delivery_enabled=true`，该兼容字段只允许保留最佳失败候选作为诊断：报告必须写 `formal_accepted=false`、`delivery_class=review_only`。Stage 8 从冻结 Stage 6 含星输入生成安全预览，Stage 9 不运行无星重混，Stage 10 只写 `result_review*`；任何 Stage 7 硬画质失败均不能占用正式结果名。
...
- Stage 8 双窄带哈勃艺术分支默认开启，由 `STARUN_STAGE8_DUALBAND_PALETTE_ENABLE=0` 显式关闭；`STARUN_OPTIONAL_COLOR_TRANSFORM` 仅保留宽带/Stage 9 可选插件授权。任务参数 `stage8_dualband_palette_selection` 默认为 `auto`，也可手动指定 `HSO/SHO/OSH/OHS/HOS/HOO`；手动值是唯一候选，拒绝后回滚而不切换色盘。处理计划一次冻结 `requested_palette / automatic_palette / palette / selection_mode / manual_override`，Stage 8 只消费该结论。该分支只读取 Stage 4/Stage 5 验签恢复的冻结映射，不得使用当前 Header 重新分类；契约、映射、色盘结论缺失或字段异常时明确跳过。双窄带 SPCC 失败后，通过安全门的降级 PCC 父本仍执行伪色映射，但不清除 `requires_review`。自动模式不做六候选评分，只执行冻结目标的第一优先级：未知但确认 Ha/OIII→HOO、Ha 主导宽场发射星云→SHO、暗星云/Ha 背景轮廓→HSO、丝状超新星遗迹→OSH、紧致 OIII 目标→OHS、亮核混合发射目标→HOS。六套色盘固定使用常规 `Classic` 通道公式 `H=R`、`O=(G+B)/2`、`S*=(H+O)/2`，并在报告中披露 `S*` 为合成代理。调色前必须保存 `stage8_pre_palette.fit`；亮度漂移和裁剪增长超过名义门限但不超过 50% 时仅写 warning 并接受，超出宽限或背景/mask 外变化则事务回滚。接受后写 `stage8_palette_selected.fit` 和 `stage8_palette_report.json`，Stage 9 禁止再运行 Vectra 二次转色。
...
- Stage 9 同域映射以 `stage7_stretch_quality.json.matched_domain_transfer / starun.stage7-matched-domain-transfer.v1` 为权威合同。任一 `cand_display70/82/90` 胜出时都必须重建并验签对应 LUT；`cand_vivid_safe` 胜出时合同同时记录 vivid 子候选和真实 tone 父候选，Stage 9 只重建父 LUT，再沿用已验证星色层。合同缺失、损坏或 SHA-256 不符时停用 Unscreen 并保留 Screen 基线，禁止改用 cand_b MTF。非 Display tone 候选使用下一条的 `closed_form_mtf_reference`。
- Stage 9 保留 Stage 6 线性 `original-starless` 分离和 Alpha+Screen 基线，不调用 SyQon 的伪 descreen。项目静态主强度上限为 `1.05`，但自动目标分析和 Stage 6/7 安全缩放仍可降低实际强度；降强度阶梯中亮星可降至 `0.40`，弱星默认不低于 `0.55`。仅当 Stage 6/7 已验收、不可变 Stage 6 pair handoff 与独立源星表可验证、且 `stage7_stretch_quality.json.closed_form_mtf_reference` 可恢复时，才用同一 linked-MTF 将 handoff 指定的 `stage6_input/stage6_starless` 映射到显示域，构造 `U=(O-B)/(1-B)`。Stage 8 改写可变 `starless_file` 不再会触发错误 pair mismatch；本轮 handoff 缺失或无效时安全跳过 Unscreen 和 PSF 闭环。正式星层只采用 U 的上行幅度并固定沿用可信 `starmask_stretched` RGB 比例，因此它是保色幅度校正候选，不是逐通道精确逆。可靠支持不足 `80%`、分母/饱和/负残差不安全时跳过。Unscreen 通过全部既有门后，还须相对基线达到 RGB MAE `>=10%` 且 `>=0.005` 的改善，并满足色度、恢复率、星翼和 advisory 非回归；否则回载 `stage9_candidate_subtraction_screen`。Screen/Unscreen 共用同星显示域 FWHM 门：普通 FWHM 只使用未饱和孤立星；参考全部/弱星/亮星组达到最低样本时，候选可测数不足或比例超出 `0.93–1.10` 均硬拒绝。至少一组低于 `0.97`、所有组均不高于 `1.05` 且没有其他非 PSF 拒绝时，才从不可变可信星层依次重建 `+1 px`、必要时 `+2 px` 的源确认星翼候选；每项复跑完整门控，按最坏分组 FWHM 偏差、平均偏差、最小扩张量、高光增长和亮像素增长排序。任一组已偏大、候选过大或出现非尺寸门拒绝都会停止扩张并恢复最佳安全候选。通过后的 Unscreen 还可生成来源完整性候选：普通星只加入 strict→正常支持之间的源确认外圈，并从算子硬上限按 `0.95 → 0.90 → 0.85 → 0.80` 递减竞争首个通过全部硬门的版本；普通目录遗漏的饱和星和 `FWHM >= 8 px` 大亮星则从 Stage 5 冻结星表与原始 starmask 交叉确认后补入，支持半径上限 `12 px`。为处理半高核心已不小、但低亮外翼仍截断的显示问题，来源完整性候选通过后只读加载原任务源（尺寸不兼容时回退已验证 `stage6_input`）并在内存执行 `autostretch -linked`，仅把同源自动拉伸的局部背景扣除 `5%` 与 `10%` 峰值连通轮廓作为可见外翼形态标尺；它不替换 Stage 7、不放宽 FWHM 硬门，也不声明科学测光保真。任一轮廓外径低于源参考目标的未饱和同星才进入恢复：`10%` 已达标时仅补 `5%–10%` 外裙，否则才允许源 `5%–45%` 轮廓；低亮轮廓自身提供局部背景/信号，不因附加半高拟合失败漏掉高背景或大亮星。恢复仍排除严格源 FWHM 星核，仅在同源轮廓和 matched-domain 可靠 raw Unscreen 共同支持内按 `+2 → +1 → +0 px` 与 `1.15 → 1.10 → 1.05 → 1.00×` 竞争；普通星可见外翼半径上限独立为 `10 px`，新增幅度按实际 Stage 8 remix base 与有效 Screen 强度逐 RGB 反算，并同时钳在源参考逐像素目标与候选局部半高线 `90%` 以下，提升低亮外径而不扩大半高核心。任何候选仍须通过原 `1.10`、高光、色彩及结构门，全部拒绝则事务恢复来源完整性候选。补入层会按实际 Stage 8 remix base 的 Screen 余量约束整个补入支持，使合成峰值不超过冻结 Stage 5 源星显示峰值；补入组只做存在/星翼观察，不进入普通 FWHM 门。参考子组本身不足则候选可保存但设置 `stage9_psf_review_required=true`，Stage 9 降级且 Stage 10 仅发布 `result_review*`。饱和星单列核心面积与星翼观察，不参与普通 FWHM 硬门。正式验收的像素回混仍记 `stars_application_mode=screen`；有界失败候选改记 `screen_review_candidate` 并只允许复核交付。
...
- Stage 9 的 `stage9_remix_quality.json` 使用 `starun.stage9-remix-quality.v9`，固定 `selection_policy=catalog_visibility_psf_fidelity_recovery_v7`，并包含 `starmask_support_preflight`（`starun.stage9` 的 v2 starmask support-preflight 契约）、`catalog_visibility` 与 `persisted_output_validation`。插件 faint/mid/bright/peak 四锚点必须各达到目标的 `50%` 才有正式资格；不足、测量缺失或坐标合同不匹配时改用同一 raw starmask/normal support 构建的内置多锚点层与 strict 分支竞争。正式候选除既有 `0.93–1.10` FWHM、色度、高光、背景、裁切和光晕门外，还必须通过源目录绝对可见度（源局部对比度 `>=0.002`；全部/弱/亮星可见率分别 `>=0.75/0.70/0.90`），保存后重新加载 `stage9_remixed.fit` 核对尺寸、像素哈希、坐标域并重跑关键门。只有持久化复验通过才设置 `stars_applied=true / remix_formally_accepted=true`；旧 v8 可读取但不能缺省提升为正式成功。正式路径失败后仍按冻结 Stage 8 Starless + 受控/原始 Starmask、最后 Stage 5 的顺序生成含星 review-only 回退；全部不可安全验证时必须 withheld，禁止把 Starless remix base 冒充最终或复核结果。
...
- Stage 9 在候选接受、全部拒绝回滚或 starmask fail-closed 且 `stage9_remixed.fit` 保存成功后，都要生成 `review_bundles/stage9_star_remixing/`；Stage 8 交接文件保持不可变，实际二次细化结果另存 `stage9_starless_base.fit` 并作为复核 before。复核包生成失败只记录警告，不改变质量门的确定性结论。四张 16-bit 工作图只服务运行期判定；最终成功阶段只保留 `review.json`，异常阶段另保留一张 8-bit 紧凑 `review.png`。
...
- Stage 10 最终质量报告使用 `starun.final-quality.v2`：始终以冻结的信号排除背景 mask 对比 Stage 10 输入/最终图，单项局部 patch、一般色噪或无显著增长的异常只写 `soft_warning` 并允许普通出图；极端值、多指标显著增长、非法图像及上游结构问题才 `hard_reject`。仅可信 Stage 9 正常源可执行一次有界色度/纹理修复，候选必须通过噪声改善、主体相关性/通量、裁剪和保存复检，任一失败恢复修复前检查点；既有 review-only 来源不得尝试修复。
...
| Stage 4 color/runtime | `python3 -m pytest -q tests/test_stage4_auto_reference.py tests/test_stage4_pcc_policy.py tests/test_runtime_capabilities.py tests/test_processing_parameters.py tests/test_pipeline_plugin_fallbacks_runtime_contracts.py tests/test_gui_runtime_modes.py`；检查物理成功跳过影子、在线未验证 90 秒、本地 300 秒、星体 256 上限、留出尾部门和精确回滚阻断 |
...
| Stage 3 background gates | `python3 -m unittest tests.test_stage3_background_sampling tests.test_processing_parameters tests.test_pipeline_stage_smoke`，并运行 `tests/test_pipeline_plugin_fallbacks_stage3_background.py`；检查三档阈值、内置优先顺序、插件开关、正式 Pareto 选优、软告警保留和全硬拒绝回滚 |
