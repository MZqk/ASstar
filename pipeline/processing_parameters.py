...
PROCESSING_PARAMETER_SPECS: Tuple[ParameterSpec, ...] = (
    ...
    ParameterSpec("optional_color_transform_enabled", 8, "Vectra 独占颜色路线", "expert", "bool"),
    ...
    _spec("stage10_presentation_color_p50_retention_min", 10, "表现色度 P50 保留", "float", 0.35, 0.90, 0.01, 2, section="quality_gate", strictness="higher_is_stricter"),
    _spec("stage10_presentation_color_p95_retention_min", 10, "表现色度 P95 保留", "float", 0.50, 0.95, 0.01, 2, section="quality_gate", strictness="higher_is_stricter"),
    _spec("stage10_presentation_opponent_energy_retention_min", 10, "对立色能量保留", "float", 0.40, 0.90, 0.01, 2, section="quality_gate", strictness="higher_is_stricter"),
    _spec("stage10_presentation_color_direction_correlation_min", 10, "色彩方向相关", "float", 0.70, 0.95, 0.01, 2, section="quality_gate", strictness="higher_is_stricter"),
    _spec("stage10_presentation_visibility_retention_min", 10, "主体可见度保留", "float", 0.60, 1.00, 0.01, 2, section="quality_gate", strictness="higher_is_stricter"),
    _spec("stage10_presentation_microdetail_retention_min", 10, "微细节保留", "float", 0.70, 1.00, 0.01, 2, section="quality_gate", strictness="higher_is_stricter"),
    _spec("stage10_presentation_microdetail_growth_max", 10, "微细节增长", "float", 1.00, 1.60, 0.05, 2, section="quality_gate", strictness="lower_is_stricter", suffix="×"),
    _spec("stage10_presentation_galaxy_brightness_retention_min", 10, "星系主体亮度保留", "float", 0.58, 1.00, 0.01, 2, section="quality_gate", strictness="higher_is_stricter"),
    _spec("stage10_presentation_generic_brightness_retention_min", 10, "普通主体亮度保留", "float", 0.60, 1.00, 0.01, 2, section="quality_gate", strictness="higher_is_stricter"),
    _spec("stage10_presentation_composite_brightness_retention_min", 10, "复合亮星云亮度保留", "float", 0.45, 1.00, 0.01, 2, section="quality_gate", strictness="higher_is_stricter"),
    _spec("stage10_presentation_star_preserve_brightness_retention_min", 10, "保星目标亮度保留", "float", 0.20, 1.00, 0.01, 2, section="quality_gate", strictness="higher_is_stricter"),
    _spec(
...
PROCESSING_GATE_PARAMETER_SPECS: Tuple[ParameterSpec, ...] = (
    ...
    _gate("stage5_multiscale_detail_retention_min", 5, "主体细节保留下限", "float", 0.90, 0.98, 0.01, 2),
    _gate("stage5_multiscale_noise_reduction_min", 5, "背景噪声下降下限", "float", 0.12, 0.50, 0.01, 2),
    ...
    _gate("stage7_subject_chroma_retention_nominal", 7, "主体色度保留目标", "float", 0.70, 1.00, 0.01, 2),
    _gate("stage7_subject_chroma_retention_hard_min", 7, "主体色度保留硬线", "float", 0.55, 1.00, 0.01, 2),
    _gate("stage7_non_background_chroma_retention_nominal", 7, "非背景色度保留目标", "float", 0.45, 1.00, 0.01, 2),
    _gate("stage7_non_background_chroma_retention_hard_min", 7, "非背景色度保留硬线", "float", 0.30, 1.00, 0.01, 2),
    _gate("stage7_star_preserve_chroma_retention_nominal", 7, "保星主体色度目标", "float", 0.35, 1.00, 0.01, 2),
    _gate("stage7_star_preserve_chroma_retention_hard_min", 7, "保星主体色度硬线", "float", 0.23, 1.00, 0.01, 2),
    _gate("stage7_low_chroma_source_saturation_max", 7, "低色度源饱和上限", "float", 0.0, 0.02, 0.001, 3),
    _gate("stage7_low_chroma_source_opponent_rms_max", 7, "低色度源对立色 RMS", "float", 0.0, 0.0001, 0.00001, 5),
    _gate("stage7_visible_noise_score_max", 7, "显示域可见噪声正式线", "float", 0.005, 0.025, 0.001, 3),
    _gate("stage7_visible_noise_score_hard_max", 7, "显示域可见噪声硬线", "float", 0.005, 0.0375, 0.0005, 4),
    ...
    _gate("stage8_large_galaxy_structure_scale_max", 8, "大型星系结构强度", "float", 0.10, 0.50, 0.05, 2),
    _gate("stage8_large_galaxy_feather_radius_ratio", 8, "大型星系羽化比例", "float", 0.004, 0.004, 0.001, 3),
    _gate("stage8_emission_structure_scale_max", 8, "发射星云结构强度", "float", 0.10, 0.45, 0.05, 2),
    _gate("stage8_emission_feather_radius_ratio", 8, "发射星云羽化比例", "float", 0.008, 0.008, 0.001, 3),
    _gate("stage8_bright_composite_structure_scale_max", 8, "复合亮星云结构强度", "float", 0.10, 0.40, 0.05, 2),
    _gate("stage8_bright_composite_feather_radius_ratio", 8, "复合亮星云羽化比例", "float", 0.010, 0.010, 0.001, 3),
    _gate("stage8_frozen_sky_visible_noise_growth_max", 8, "冻结天空可见噪声增长", "float", 1.00, 1.10, 0.01, 2, suffix="×"),
    _gate("stage8_palette_subject_saturation_input_ratio_min", 8, "调色主体饱和相对下限", "float", 0.50, 1.00, 0.01, 2),
    _gate("stage8_palette_subject_saturation_absolute_min", 8, "调色主体饱和绝对下限", "float", 0.08, 0.30, 0.01, 2),
...
_GATE_PROFILE_EXCLUDED_FIELDS = frozenset(
    {
        ...
        "stage7_subject_chroma_retention_nominal",
        "stage7_subject_chroma_retention_hard_min",
        ...
        "stage8_large_galaxy_feather_radius_ratio",
        "stage8_emission_structure_scale_max",
        "stage8_emission_feather_radius_ratio",
        "stage8_bright_composite_structure_scale_max",
        "stage8_bright_composite_feather_radius_ratio",
