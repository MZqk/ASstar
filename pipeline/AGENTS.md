...
约束 `pipeline/seestar_Superimpose.py`（stage 1-10）和 `pipeline/stage11_ai_postprocess.py`（可选 Stage11）。

## Principles

- 默认保守：保护数据真实性优先于亮度、饱和度和“好看”。
- 顺序遵循线性 -> 非线性：背景/校色/线性降噪 -> 拉伸/去星/星云增强/星点回混/导出。
- 禁止默认引入明显破坏真实性的行为：大面积纯黑背景、星色失真、饱和溢出、弱信号被抹除。
- 激进增强必须可选，默认关闭或低风险。

## Stage Contract

- `stage1` ~ `stage10` 是稳定主流程，不得随意改序。
- 必须改序时：说明原因、收益、副作用、兼容策略，并同步 README 流程描述。
- 每阶段需有输入假设、失败降级路径（`degraded`/`skipped`）和可定位日志；非致命失败不应阻断主产物导出。
