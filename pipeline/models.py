...
class PipelineConfig:
    ...
    stage4_auto_reference_global_white_enabled: bool = True  # 自动矩形白参考默认启用；关闭后仅允许背景中和
    ...
    stage9_sep_axis_ratio_min: float = 0.50  # 独立 SEP 正式星表允许的最小 b/a；仅用于持久化同源门
    stage9_sep_fwhm_ratio_min: float = 0.50  # 独立 SEP 星径相对冻结 Stage9 FWHM 的下限
    stage9_sep_fwhm_ratio_max: float = 2.20  # 独立 SEP 星径相对冻结 Stage9 FWHM 的上限
    stage9_sep_catalog_count_min: int = 32  # O/C 独立 SEP 正式证据所需的最少有效源数
    stage9_sep_high_confidence_fraction: float = 0.20  # C 中进入正式同源门的最亮 flux 比例
    stage9_sep_high_confidence_count_min: int = 16  # C 正式高置信集合的最少源数
    stage9_sep_match_radius_fwhm: float = 0.75  # 独立 SEP 匹配半径相对冻结 FWHM 的比例
    stage9_sep_match_radius_min_px: float = 2.0  # 独立 SEP 匹配半径下限
