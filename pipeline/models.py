...
class PipelineConfig:
    ...
    bg_std_worsen_ratio_max: float = 1.10  # 旧调用方兼容字段；不再决定生产 Stage3 接受/拒绝
    bg_median_drop_ratio_min: float = 0.20  # 旧调用方兼容字段；背景零点平移不再视为源损失
    bg_object_preserve_ratio_min: float = 0.40  # 旧调用方兼容字段；不再作为生产硬门
    bg_edge_black_rise_max: float = 0.35  # 旧调用方兼容字段；不再作为生产硬门
    bg_star_preserve_ratio_min: float = 0.90  # 旧调用方兼容字段；星点保留改为诊断量
    bg_nebula_mean_change_max: float = 0.10  # 旧调用方兼容字段；改用天空面参考的目标通量/形态
    ...
    stage3_safe_sample_min_count: int = 16  # 低于该数量或覆盖不足时禁止 subsky -existing，避免稀疏误拟合
    ...
    stage3_compound_min_sample_count: int = 32  # Polynomial→残差RBF复合降级候选所需的已审计安全样点下限
    stage3_compound_fit_min_count: int = 24  # 复合候选确定性拟合集最小样点数
    stage3_compound_validation_min_count: int = 8  # 复合候选冻结验证集最小样点数
    ...
    stage4_spcc_enabled: bool = True  # Stage4 物理校色首选 SPCC；宽带异常时才回退 PCC
    stage4_spcc_timeout_sec: int = 180  # SPCC 单次独立 Siril 子进程超时；不在同一进程中重试
    stage4_spcc_osc_sensor: str = "Sony IMX585"  # Seestar S30 Pro OSC 传感器的 Siril SPCC 数据库名称
    stage4_spcc_osc_filter: str = ""  # 为空时按滤镜提示选择 ZWO Seestar LP / No filter
    ...
    stage4_pcc_timeout_sec: int = 180  # SPCC 异常后的宽带 Gaia PCC 只尝试一次；达到该秒数即终止
    ...
    stage7_starless_linked_mtf_preview_p50_ratio: float = 0.70  # 可视背景相对 linked preview P50 的目标比例
    stage7_starless_linked_mtf_p50_max: float = 0.22  # 可视背景上限，避免为提亮弱信号而过度拉伸
    ...
    stage7_diffuse_visibility_score_min: float = 0.08  # 宽场/暗星云正式候选的最低弱信号可见度，避免近黑候选进入 Stage 8
