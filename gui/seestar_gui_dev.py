...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "不打包运行 Seestar 源码 GUI，并显式使用系统 Siril 与现有 "
            "Siril Python seed。"
        )
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "可选的 Seestar runtime HOME；不传时继续使用 "
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="seestar_gui_dev_resources_") as td:
        ...
        try:
            from .main_window import QApplication, SeestarGui
        except ImportError:  # Direct execution: python gui/seestar_gui_dev.py
